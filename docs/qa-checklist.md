...
| **육성 난장(라이브)** | ⛔ | ⛔ | ⛔ |  <!-- 오디오·SFU 라 실기기 필요. 운영 데이터 open_rooms 0행·is_live 0건. CF Calls 시크릿도 미설정 상태 -->
| **삐삐 개통(랜덤)** | ✅ | ❌ | ❌ |
| 삐삐 번호 골라 개통 | ✅ | ❌ | ❌ |  <!-- 0902 롤백 시뮬: 7~8자리 숫자만 허용(bad_format), '0121004' → 012-012-1004 개통, 같은 사람 재시도는 같은 번호(멱등), 남이 같은 번호 요청하면 taken -->
| **삐삐 음성 발신·수신** | ⛔ 마이크 필요 | ❌ | ❌ |
...
| **후원(발의자·크리에이터)** | ⛔ 결제성 | ⛔ | ⛔ |
| **출금 요청** | ⛔ 결제성 | ⛔ | ⛔ |
| 이용권 시트 — 가격 미노출 | ✅ | ✅ | ✅ |  <!-- 0902 코드 확인: 네이티브는 스토어 오퍼가 있을 때만 가격·구매버튼 표시(IAP 미등록이면 둘 다 안 뜸) = anti-steering 안전. 웹은 원화 가격+결제버튼 정상 -->
...
| 항목 | 상태 |
|---|---|
| **다국어(i18n · GALLA_t · locale 컬럼)** — 번역 누락·깨짐 | 🔶 인프라는 있다 — `js/i18n.js` 사전 **1,779키**, `translations` 표, `enabled_locales()` RPC. 다만 **활성 언어가 `ko` 하나**뿐이고 콘텐츠 locale 도 전부 ko 라 번역 깨짐을 실사용으로 볼 수 없다 |
| translate 엣지 함수 | ❌ |
...
| 에러 상태(네트워크 실패·403·404) | 🔶 **여행지만 진짜 404**(`/travel/none-…` → 404). 임의 경로(`/xyz-not-exist`)와 **없는 이슈(`/issue?id=99999999`)는 200**(SPA 폴백=소프트 404) — 미들웨어 주석이 스스로 경고한 그 문제가 이슈엔 남아 있다. 403 은 RPC 레벨에서 `forbidden` 확인(§26-17) |
| 긴 텍스트·이모지·RTL 깨짐 | 🔶 운영 데이터 최장이 이슈 제목 56자·본문 611자·댓글 178자라 **깨짐을 볼 만한 길이가 없다**. 인위로 긴 문자열·이모지를 넣어 화면을 봐야 한다 |
| 동시성(같은 계정 2기기) | 🔶 같은 계정 **2탭**: 한쪽이 토큰을 갱신하면 다른 쪽도 즉시 새 토큰을 쓰고 쿼리 정상 — **강제 로그아웃 없음**. A탭 알림 삽입 → **B탭 4초 뒤 조회 1건·뱃지 '1'**. ⚠️ 같은 브라우저는 storage 공유라 **진짜 2기기가 아니다** — 앱 로그인 뒤 웹+앱으로 마무리해야 한다 |
...
| 항목 | 상태 |
|---|---|
| Safari(iOS·macOS) | 🔶 실브라우저 없이 **기능 요구치로 대조**: 코드가 `color-mix()`(Safari 16.2+)·`:has()`·`dvh`·`crypto.randomUUID`(15.4+)를 쓴다. **Safari 16.2 이상이면 문제 없고 그 미만은 깨진다**(§26-29) |
| Chrome / Edge / Firefox | 🔶 Chrome 만 |
| 삼성 인터넷 | 🔶 Chromium 기반이라 위 기능은 최신판에서 전부 지원. 실제 기기 확인은 남음 |
| iOS 버전 하한(구형 기기) | ✅ **16.2 로 올림**(2026-09-03 사장님 승인) — 코드가 요구하는 값과 일치. `project.pbxproj` 6곳 + `CapApp-SPM` platforms `.v16`. 빌드 성공·산출 `MinimumOSVersion=16.2`·시뮬 재설치 기동 확인 |
| 안드로이드 버전 하한 · 저사양 | 🔶 `minSdkVersion 24`(Android 7). WebView 는 별도 업데이트되니 최신 WebView 면 지원되지만, **업데이트 안 된 구형 WebView 에서는 iOS 와 같은 이유로 깨질 수 있다** |
| 태블릿·폴더블 | ✅ 768×1024·1024×768·374×818(폴드 접힘) 전부 **가로 스크롤 0**, 하단 네비도 화면 안(374 폭에서 330px). 넘치는 요소는 카테고리 칩(가로 스크롤 컨테이너 내부라 정상) |
| 느린 3G · 오프라인 전환 | 🔶 SW **active**, 캐시 버킷 2개에 **911개 자산**, `offline.html` 캐시 확인. 실제 오프라인 전환·3G 스로틀은 도구가 없어 미검증 |
...
### 26-27. 품질축 (2026-09-02)

| 항목 | 결과 |
|---|---|
| 다국어 | 🔶 사전 1,779키·`translations` 표·`enabled_locales()` 존재. **활성 언어 `ko` 하나**라 실사용 검증 불가 |
| translate 엣지함수 | ✅ (§10-C — uid 필수 + `translate_gate` 한도) |
| 에러 상태 | 🔶 **여행지만 진짜 404**. `/xyz-not-exist`·`/issue?id=99999999` 는 **200**(소프트 404) |
| 긴 텍스트·이모지 | 🔶 운영 데이터 최장 56/611/178자 — 검증할 길이가 없다 |
| 동시성(2기기) | ❌ 기기 하나로는 못 본다 |

⚠️ **또 하나의 측정 함정**: `curl -I`(HEAD)로 재면 `/travel/none-…` 이 **200 으로 보인다**. GET 으로 재야
404 가 나온다. 오늘만 리다이렉트 미추적(§26-26)에 이어 두 번째 — **상태코드는 `curl -sL -o /dev/null -w '%{http_code}'` 로 잰다.**

⚠️ 소프트 404 는 제품 판단이 필요하다. 여행지는 "없는 곳이면 진짜 404"로 이미 정했는데, 이슈에 같은
규칙을 적용하면 **앱의 뷰 로더가 없는 id 를 404 로 받아 '불러오지 못했어요'** 를 띄우게 된다(그게 맞는
동작이긴 하다). 색인 품질 vs 앱 동선 — 사장님 결정으로 남긴다.

### 26-28. 삐삐 번호·이용권 시트·육성 난장 (2026-09-02)

| 항목 | 결과 |
|---|---|
| 삐삐 번호 골라 개통 | ✅ 7~8자리 숫자만 허용 · `0121004` → **012-012-1004** 개통 · 같은 사람 재시도는 **같은 번호**(멱등) · 남이 같은 번호 요청하면 **`taken`** |
| 이용권 시트 | ✅ 네이티브는 **스토어 오퍼가 있을 때만** 가격·구매버튼을 그린다(IAP 미등록이면 둘 다 안 뜸) → anti-steering 안전. 웹은 원화 가격+결제버튼 정상 |
| 육성 난장(라이브) | ⛔ 오디오·SFU 라 실기기 필요. `open_rooms` 0행·`is_live` 0건, CF Calls 시크릿도 미설정 |

**기본 OG 카드 배포 반영 확인**(§26-24·26-26 마무리): `/quest`·`/grade` 에 `og-default.png` 주입됨.
고유 OG 가 있는 곳은 그대로다 — `/match` 는 전용 `match/intro.png`, 이슈 상세는 콘텐츠 이미지.
**중복 주입 없음**(전부 og:image 1개).

### 26-29. 호환성 6건 — **앱 최소 iOS 와 코드 요구가 어긋난다** (2026-09-02)

실브라우저(사파리·삼성인터넷)가 없어 **코드가 요구하는 최소 버전을 역산**했다.

| 쓰는 기능 | 사용 수 | 요구 버전 |
|---|---|---|
| `color-mix()` | 19곳 (폴백 없음) | **Safari 16.2** / Chrome 111 |
| `:has()` | 17곳 | Safari 15.4 / Chrome 105 |
| `dvh` 단위 | **38곳 (폴백 없음)** | Safari 15.4 / Chrome 108 |
| `crypto.randomUUID` | 4곳 | Safari 15.4 — **두 곳 다 폴백 있음** ✅ |
| `requestIdleCallback` | 4곳 | 사파리 미지원 — **폴백 있음** ✅ |

**🔴 앱 배포 타깃은 `IPHONEOS_DEPLOYMENT_TARGET = 15.0`.**
즉 **iOS 15.0~16.1 기기는 앱을 설치할 수 있는데 화면이 깨진다** —
`min-height:100dvh` 가 무효라 로그인·작성·보관 화면의 높이 기준이 사라지고(`login.css:17` 등),
`color-mix()` 색이 무효라 등급·작성 화면의 테두리·배경이 사라진다.

고르는 길 둘:
- **(a) 배포 타깃을 16.2 로 올린다** — 코드가 실제로 요구하는 값에 맞춘다. 한 줄. iOS 15 점유율은 미미하다.
- (b) 폴백 57곳을 넣는다(`100vh` 병기·`color-mix` 대체색) — 노동이 크고 새 코드마다 반복된다.
→ **(a) 로 결정(2026-09-03 승인).** `project.pbxproj` 6곳 + `CapApp-SPM` `.v16`.
  검증: 빌드 성공(46초) · 산출 `Info.plist MinimumOSVersion=16.2` · 시뮬레이터 재설치 후 정상 기동.

안드로이드는 `minSdkVersion 24`(Android 7). WebView 가 최신이면 문제없지만 업데이트 안 된 구형 WebView 는 같은 이유로 깨진다.

**태블릿·폴더블** ✅ 768×1024 · 1024×768 · 374×818 전부 가로 스크롤 0, 하단 네비도 화면 안.
**오프라인** 🔶 SW active · 911개 자산 캐시 · `offline.html` 캐시 확인(전환 실동작은 도구 없어 미검증).
