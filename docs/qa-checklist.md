...
| iOS 버전 하한(구형 기기) | ❌ **불일치 발견** — 앱 배포 타깃은 `IPHONEOS_DEPLOYMENT_TARGET = 15.0` 인데 코드는 **16.2** 를 요구한다(`color-mix` 19곳·`dvh` 38곳에 폴백 없음). **iOS 15.0~16.1 기기는 설치는 되고 화면이 깨진다**(§26-29) |
...
→ **(a) 를 권한다. 설치 가능 기기 범위를 바꾸는 결정이라 사장님 확인 뒤 올린다.**
