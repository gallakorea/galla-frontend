...
The two filters above the video list carry official marks as well: the official
YouTube icon for "Long-form", and the official YouTube Shorts asset for "Shorts" —
both used exactly as supplied, unmodified, at 20dp.

The only icons left that previously carried the hand-drawn mark are two write-shortcut
icons in our secondary navigation. Those label our own posting shortcuts, not YouTube,
so we removed the play symbol from them entirely and use plain landscape / portrait
rectangles that convey aspect ratio. Nothing in our interface resembles a YouTube mark
unless it is an official asset.
