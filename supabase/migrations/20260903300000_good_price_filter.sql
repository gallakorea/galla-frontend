-- 🏷 착한가격 전용 필터 — 목록·지도 양쪽에서 '착한가격업소만' 보기.
--
-- ⚠️ food_map 은 인자를 더하면 **새 오버로드**가 생기고, 파라미터 이름이 겹쳐
--    PostgREST 가 못 고른다(42725 not unique) — 앱이 통째로 죽는다. 어제 실제로 죽였다.
--    그래서 기존 시그니처를 **드롭하고** 한 트랜잭션에서 다시 만든다.
--    새 인자는 반드시 **맨 뒤에** 붙이고 기본값을 준다(기존 호출부는 그대로 동작).
drop function if exists food_map(numeric,numeric,numeric,numeric,text,text,boolean,integer,text,integer,boolean);

CREATE OR REPLACE FUNCTION public.food_map(p_sw_lat numeric DEFAULT NULL::numeric, p_sw_lon numeric DEFAULT NULL::numeric, p_ne_lat numeric DEFAULT NULL::numeric, p_ne_lon numeric DEFAULT NULL::numeric, p_region text DEFAULT NULL::text, p_channel text DEFAULT NULL::text, p_only_unvisited boolean DEFAULT false, p_limit integer DEFAULT 300, p_category text DEFAULT NULL::text, p_min_shows integer DEFAULT NULL::integer, p_spread boolean DEFAULT false, p_good_price boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE sql
