-- 🍚 혜자식당에서 착한가격을 뺀다 — 두 축을 진짜로 가른다.
-- 시그니처는 그대로, 본문만 바꾼다(인자를 더하면 오버로드가 생겨 PostgREST 가 못 고른다).
  ...
  from (
    ...
    where p.status = 'live'
      ...
      /* 🍚 혜자식당 = 외부에서 받은 목록(씨앗) + 우리가 가진 싼 집.
         ...
         🔴 **착한가격은 뺀다.** 정부가 실사해 지정한 집은 그 자체로 다른 축이고,
            안 빼면 혜자식당의 80%가 착한가격으로 채워져 두 카테고리가 같아 보인다
            (실측 2026-09-04: 둘 다 6,010 · 혜자만 1,453 — 열어도 아까 본 집이 또 나온다).
            데이터가 겹치는 건 상관없지만 **보여주는 축은 갈라야** 한다. */
