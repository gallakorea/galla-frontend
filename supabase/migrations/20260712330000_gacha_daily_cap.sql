-- 가차 하루 제한(과몰입·남용 방지) — 하루 30회. gacha_draw에 카운트 게이트 추가
...
begin
  ...
  if v_today >= c_daily then
    return jsonb_build_object('ok',false,'reason','daily_limit','limit',c_daily,'used',v_today);
  end if;
