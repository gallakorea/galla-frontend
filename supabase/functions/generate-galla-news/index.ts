...
Deno.serve(async (req) => {
  ...
  const MAX_TOPICS = Math.min(body.max_topics ?? 5, 8);
  ...
  const cluster = await chat([
    { role: "system", content: "너는 한국 뉴스 데스크 편집자다. 헤드라인 목록에서 지금 뉴스가치가 높은 주제를 골라 같은 사건끼리 묶는다. 광고성/단순 홍보/가십은 제외. JSON만 출력." },
    { role: "user", content: `헤드라인:\n${headlines}\n\n지침: 같은 사건/주제인 헤드라인의 index를 묶어 상위 ${MAX_TOPICS}개 주제를 골라라. 소스가 2개 이상인 주제를 우선한다. category는 [정치,경제,사회,세계,IT과학,문화,스포츠,연예] 중 하나.\n형식: {"topics":[{"topic":"핵심 주제 한 줄","category":"정치","members":[0,5]}]}` },
  ], 700);
  ...
  for (const t of topics.slice(0, MAX_TOPICS)) {
    ...
    const gen = await chat([
      { role: "system", content: "너는 갈라뉴스 기자다. 여러 언론사 보도를 종합해 사실 기반의 새 기사를 쓴다. 규칙: (1) 특정 기사 문장을 그대로 베끼지 말고 네 표현으로 다시 써라. (2) 제공된 자료에 없는 사실은 지어내지 마라. (3) 중립적이고 담백하게. JSON만 출력." },
      { role: "user", content: `주제: ${t.topic}\n\n자료:\n${sourceText}\n\n형식: {"title":"제목","summary":"한 줄 요약","category":"${t.category || "사회"}","body":"3~5문단, 각 문단은 빈 줄로 구분"}` },
    ], 900);

    if (!gen?.title || !gen?.body) continue;
    ...
    if (error || !ins) { results.push({ topic: t.topic, error: error?.message }); continue; }
    ...
    results.push({ topic: t.topic, id: ins.id, sources: members.length });
  }
