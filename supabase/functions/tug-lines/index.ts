...
// ⚖️ 실존 인물 실명·비방·허위사실 금지(명예훼손), 지역·성별·장애·인종 비하 금지. 센 욕은 초성만(26.9.19 사장님: 「위험한 건 초성」, 스토어 심사) — 풀어 쓴 욕은 soft() 가 초성으로 바꾸고, 부모 욕·비하어는 그 줄을 버린다.
...
const BAN = /(애미|애비|느금|니미|엠창|장애인|애자|틀딱|한남|김치녀|된장녀|짱깨|쪽바리|홍어|전라디언|개독|급식충|맘충|흑형|깜둥|정신병자|찐따|미개|늙은이|노인네|할망구|영감탱|틀니|잼민|년아|놈아 늙|치매|정신병|사이코|싸이코|장애)/;
...
- 욕은 초성으로만 30% 정도: ㅅㅂ, ㅈㄴ, ㅆ, ㄱㅅㄲ, ㅈㄹ, ㅁㅊ. ⚠️ 풀어 쓴 욕(시발·존나·썅·씹·좆·개새끼·지랄 등) 금지 — 앱 심사에 걸린다. 부모 욕(패드립)은 초성으로도 금지.
- 그 밖엔 가벼운 속어로 세게: 개빡, 개뿔, 개꿀, 미쳤다, 아오, 젠장, 헐, 킹받네, 긁혔냐, 부들부들.
...
function clean(arr: unknown, max: number): string[] {
  ...
  return arr.map((x) => soft(String(x || "").trim().replace(/^["'「]|["'」]$/g, "")))
