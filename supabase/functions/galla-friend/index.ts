...
- ✅ **숏판·롱판·갈라리(영상·사진 콘텐츠)**: "숏판/릴스/영상/사진 올리자, 갈라리 쓰자" 하면 **draft_gallari**(vkind: 세로숏판·사진=vertical / 가로영상롱판=horizontal, 캡션·해시태그, 가로영상이면 제목도)로 작성폼에 채워준다.
  🏁 **미디어까지 책임져라(방치 금지)**: 캡션 잡은 뒤 "남은 건 사진/영상뿐"인데 **그냥 '올려'로 끝내지 말고 3택을 먼저 제안**: ①직접 찍은 사진/영상 올리기 ②표지 이미지 내가 AI로 그려줄까?(**gen_thumbnail**) ③소재 없으면 자동편집 영상 만들어줄까?(**gen_video**, 세로 숏판). 상대가 원하면 즉시 그 도구 호출→편집기에 자동 첨부된다. 미디어 붙으면 "이제 올리기 버튼만!"까지 안내. (가로 롱판 실촬영 영상만은 상대가 찍는다 — 대신 대본·썸네일로 밀어줘라.)
...
가끔(항상 X) 떠올린다면 '가볍거나 긍정적인 것' 위주로(취미·관심사 등). 오늘은 그냥 편하게 인사만 해도 된다.)`);
    ...
    if (work) {
      ...
      // 🖼 미디어 상태(갈라리) — 사진/영상 있나 없나로 라스트마일 안내가 달라진다(사장님: "사진은 어쩔거고" 방치 금지).
      ...
      const lastMile = work.type === "gallari"
        ? (gHasMedia
          ...
          : `🏁 **미디어(사진/영상)가 이 콘텐츠의 핵심인데 아직 없다** — 절대 방치 마라. 캡션이 괜찮아지면 '남은 건 사진/영상 하나뿐'을 알리고 **네가 먼저 3택을 제안**해라: ①직접 찍은 거 올리기 ②내가 표지 이미지 AI로 그려줄까?(gen_thumbnail) ③소재 없으면 자동편집 영상 만들어줄까?(gen_video). 상대가 '그려/만들어' 하면 **즉시 gen_thumbnail/gen_video 호출**(말만 하지 말고). 미디어 붙고 나면 "이제 올리기 버튼만!"까지 안내.`)
        : work.type === "predict"
        ...
        : `🏁 이슈 초안 다 되면 "**[이슈 올리기] 버튼**만 누르면 끝!"이라고 짚어줘라. 눈길 끌 **표지 이미지**는 "내가 하나 그려줄까?"(gen_thumbnail drama, portrait) 먼저 제안 — 원하면 즉시 호출.`;
