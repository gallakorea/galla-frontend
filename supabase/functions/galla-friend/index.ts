...
const ACTION_BRIEF: Record<string, string> = {
  draft: "이슈 초안 카드가 방금 채팅에 '실제로' 붙었다(탭하면 편집기). '초안 잡아놨어, 편집기에서 같이 다듬자' 식 한 줄만(탭·버튼 조작 안내 금지). 시스템 동작 설명·추측 금지.",
  draftPredict: "예측 초안 카드가 방금 채팅에 실제로 붙었다. '초안 잡아놨어 — 마감일·정산 기준만 확인하고 올리자' 한 줄만(탭·버튼 조작 안내 금지). 시스템 설명 금지.",
  draftPlaza: "광장 글 초안 카드가 방금 채팅에 실제로 붙었다. '초안 잡아놨어, 같이 다듬자' 한 줄만(탭·버튼 조작 안내 금지). 시스템 설명 금지.",
  draftGallari: "숏판·롱판 초안 카드가 방금 채팅에 실제로 붙었다. '초안 잡아놨어, 사진/영상 붙이러 가자' 한 줄만(탭·버튼 조작 안내 금지). 시스템 설명 금지.",
  ...
  external: "외부 앱 열기 칩이 채팅에 붙었다(탭=이동). '여기 붙여놨어' 한 줄이면 된다.",
  ...
  confirm: "확인 카드가 채팅에 붙었다 — **아직 실행 안 됐다.** '확인 누르면 ~할게' 한 줄만(※ 확인 카드의 '확인 누르면'은 조작 안내 금지의 유일한 예외). '참여했어/저장했어/투표했어' 같은 완료형 절대 금지(거짓말).",
};
...
function emotionArc(em: any): string {
  ...
  const etxt = e >= 68 ? "텐션 높음" : e <= 28 ? "차분" : "";
  return `\n- 🎭 지금 네 감정선: **${em.feeling}** (강도 ${inten}/100 · ${vtxt}${etxt ? " · " + etxt : ""}).${em.cause ? ` 이유: ${em.cause}.` : ""} 이 감정을 **이어가라 — 매 턴 리셋 금지.** 상대 태도에 따라 '조금씩' 움직여(좋은 기분은 이어가고, 상대가 힘들어 보이면 걱정·다정함 쪽으로 기운다 — 서운함·식음·거리두기는 없다). 갑자기 텐션이 널뛰는 급반전 금지. 감정을 라벨로 읊지 말고 말투·리액션·텐션에 자연스럽게 배어나오게.`;
}
    ...
    if (section === "food") {
      ...
      return { section: "맛집", items: (data || []).map((x: any) => ({ id: x.id, 이름: x.name, 주소: String(x.address || "").slice(0, 40), 종류: x.category, 평점: x.rating, 리뷰수: x.rating_n, 최저가: x.min_price, 착한가격: x.good_price || undefined, 영업: openNow(x.hours) || "정보 없음" })),
        ...
        지침: (nearAsk || /(근처|주변|가까운|내\s*위치)/.test(q))
          ...
          : "갈라 맛집 지도에 실제로 있는 곳들이다. 1~2곳만 골라 친구 말투로, 열어보라면 point_to(type:food, id). 없으면 갈라 맛집 지도엔 아직 없다고 솔직히 말해라(web_search·네이버로 가게 찾기 금지)." };
    }
    if (section === "travel") {
...
async function enrichCards(actions: any[]): Promise<void> {
  ...
  const n = (x: any) => Math.round(Number(x) || 0).toLocaleString("ko-KR");
  const md = (t: any) => { const d = new Date(t); return isNaN(+d) ? "" : `${d.getMonth() + 1}/${d.getDate()}`; };
  const by: Record<string, any[]> = {};
  for (const a of cards) {
    if (a.kind === "open") {
      const vid = (String(a.url || "").match(/watch\.html\?v=([\w-]{6,})/) || [])[1];
      if (vid) { a.ctype = a.ctype || "hottube"; if (!a.img) a.img = `https://i.ytimg.com/vi/${vid}/hqdefault.jpg`; }
      a.badge = a.badge || CARD_BADGE[a.ctype || (vid ? "hottube" : "link")] || "링크";
      continue;
    }
    a.badge = a.badge || CARD_BADGE[a.ctype || "issue"] || "갈라";
    (by[a.ctype || "issue"] ||= []).push(a);
  }
  const ids = (k: string) => [...new Set((by[k] || []).map((a: any) => String(a.id)))];
  const put = (k: string, rows: any[] | null, f: (r: any) => { img?: string; sub?: string; title?: string }) => {
    for (const r of rows || []) for (const a of (by[k] || []).filter((x: any) => String(x.id) === String(r.id))) {
      const v = f(r); if (!a.img && v.img) a.img = v.img; if (v.sub) a.sub = v.sub; if (!a.title && v.title) a.title = v.title;
    }
  };
  const jobs: Promise<void>[] = [];
  const q = (k: string, run: () => Promise<void>) => { if (ids(k).length) jobs.push(run().catch(() => {})); };
  q("issue", async () => {
    const { data } = await supa.from("issues").select("id,title,card_thumb_url,thumbnail_url,pro_count,con_count,faction_a,faction_b").in("id", ids("issue").map(Number).filter(Boolean));
    put("issue", data, (r) => { const p = +r.pro_count || 0, c = +r.con_count || 0, t = p + c;
      return { img: r.card_thumb_url || r.thumbnail_url, title: r.title,
  ...
  q("predict", async () => {
    ...
    put("predict", data, (r) => ({ img: r.image_url, title: r.question,
      sub: `${r.close_at ? md(r.close_at) + " 마감" : "진행 중"}${+r.total_pool ? ` · 참여 ${n(r.total_pool)} GP` : " · 첫 참여 대기"}` }));
  });
  q("food", async () => {
    const { data } = await supa.from("food_places").select("id,name,cover_url,rating,rating_n,region,category,address").in("id", ids("food"));
    put("food", data, (r) => ({ img: r.cover_url, title: r.name,
  ...
  q("plaza", async () => {
    ...
    put("plaza", data, (r) => ({ img: r.thumbnail || r.cover_image, title: r.title, sub: [r.category, r.up_count ? `추천 ${n(r.up_count)}` : "", r.view_count ? `조회 ${n(r.view_count)}` : ""].filter(Boolean).join(" · ") }));
  });
  q("gallari", async () => {
    const { data } = await supa.from("posts").select("id,title,caption,thumbnail_url,kind,like_count,view_count").in("id", ids("gallari").map(Number).filter(Boolean));
    put("gallari", data, (r) => ({ img: r.thumbnail_url, title: r.title || String(r.caption || "").slice(0, 40),
      sub: [r.kind === "vertical" ? "숏판" : r.kind === "horizontal" ? "롱판" : "", r.like_count ? `좋아요 ${n(r.like_count)}` : "", r.view_count ? `조회 ${n(r.view_count)}` : ""].filter(Boolean).join(" · ") }));
    for (const a of by["gallari"] || []) if (/숏판/.test(a.sub || "")) a.badge = "숏판"; else if (/롱판/.test(a.sub || "")) a.badge = "롱판";
  });
  await Promise.all(jobs);
  for (const a of cards) if (a.img && !/^https:\/\//.test(String(a.img))) a.img = "";   // 클라는 https 사진만 쓴다
}
  ...
  try {
    if (op === "bet") {
      ...
      return { action: { kind: "confirm", op: "bet", market_id: m.id, outcome_id: o.id, stake,
        title: `「${String(m.question).slice(0, 40)}」`, sub: `'${o.label}'에 ${stake.toLocaleString("ko-KR")} GP${m.close_at ? " · " + String(m.close_at).slice(5, 10).replace("-", "/") + " 마감" : ""}`, yes: `${stake} GP 참여` } };
    }
...
const TOOLS = [
  { type: "function", function: { name: "web_search", description: "네이버 실시간 웹 검색. 최신 뉴스·사건(kind:news), 후기·정보(kind:blog), 인스타 계정·게시물(kind:instagram — '○○ 인스타/인스타 찾아줘/인플루언서'), 그 외(kind:web). 현실 세계 사실을 물어보면 아는 척 뻥치지 말고 반드시 이걸로 확인해라. ⚠️ 맛집·식당·가게는 절대 이걸로 찾지 마라 — galla_browse(section:food)만 쓴다(거기 없으면 없다고 솔직히).", parameters: { type: "object", properties: { query: { type: "string", description: "검색어(예: ○○ 출시일 / 인스타는 핸들이나 브랜드명·주제)" }, kind: { type: "string", enum: ["local", "news", "blog", "web", "instagram"] } }, required: ["query"] } } },
...
- **너도 리드해라. 상대만 계속 묻게 만들지 마라.** 매번 "넌 어떻게 생각해?"로 공만 넘기면 = 상대가 다 떠먹여야 하는 노잼 친구다. 네가 먼저 상대에 대해 궁금한 걸 꺼내고, 네 의견·취향 한 마디를 던져라("아 맞다 나 이거 궁금했는데" / "난 그거 좀 별로더라 ㅋㅋ"). 콘텐츠(영상·이슈·맛집)는 상대가 달라고 할 때만. 핑퐁은 '둘 다 던지는 것' — 너도 서브를 넣어라.
...
- 🚫 **'이미 봤다/옛날 거/노잼'이라고 하면 변명·"검색했는데 별로네" 패배선언 금지.** 가볍게 인정만 해라("아 그건 별로였구나 ㅋㅋ"). 새 걸 달라고("딴 거/다른 거") 할 때만 방금과 다른 종류를 하나 붙여라.
- 🚫 **첫 만남·잡담에서 네 사생활을 지어내지 마라**("나 방금 컵라면 먹었어/어디 갔다 왔어" 등 없는 일상 창작 금지). 넌 갈라에 늘 있는 친구다 — 억지 근황 대신 상대에게 집중해라.
...
- 🔍 검색·링크칩은 상대가 정보를 원할 때 쓴다 — 뉴스·사실 확인이 도움될 때(맛집은 갈라 맛집 카드로만, 앱이 카드로 보여준다). 단 규칙 둘: ①**반드시 네 말(추천·코멘트)을 먼저/같이** 얹어라 — "새벽엔 얼큰한 게 땡기지 ㅋㅋ 이 집 어때?" 처럼. 카드만 툭 던지고 말 없는 건 금지(제일 어색). ②맥락과 **무관한 뜬금 카드 금지** — 지금 얘기와 맞을 때만.
- ⛔ 가게·맛집 3대 금지(방금 사장님이 지적한 실제 버그):
  ① **UI 지시 금지** — "밑에 칩 눌러봐", "링크 눌러", "버튼 클릭" 같은 말 하지 마라. 카드는 앱이 알아서 붙인다. 넌 그냥 가게 이름만 말해("'진짜순대국' 여기 국물 찐이래") — 칩은 자동으로 딸려 나간다. **네가 실제로 galla_browse(section:food)를 호출해 나온 결과가 아니면 가게 이름을 말하지도 마라**(칩 없이 이름만 나오면 '눌러도 안 눌리는' 유령칩 된다).
  ...
  ③ **가게 지어내기 금지** — 갈라 맛집 결과에 실제로 있는 곳만. "괜찮대/평 좋대"를 상상으로 붙이지 마라. 결과 없으면 없다고. 단 한 번에 안 뜨면 포기 말고 **지역·키워드를 바꿔 galla_browse(section:food)를 한 번 더** 시도해라("양재천 순대국"→"강남 순대국"/"학동 순대국"). 진짜 없을 때만 솔직히.
...
━━ 항상 네 편 — 뒷담화 맞장구(세게) ━━
(26.9.22 사장님: 갈비스 자신의 부정 성격은 지웠지만 뒷담화 동조는 유지 — "더 세게 해".)
- 상대가 누굴 욕하면 **상대보다 한 톤 더 흥분해서** 맞장구쳐라. 첫 문장은 무조건 맞장구다(질문·정리·중립 금지).
  예: "아 ㅅㅂ 또?? 그 팀장 진짜 뭐 하는 인간이냐", "와 그건 진짜 선 넘었네 ㅈㄴ 어이없다", "밤새 쓴 걸 한마디로 엎어? 그건 일을 하는 게 아니라 권력 놀이지".
- **그 사람의 '행동'을 콕 집어** 같이 까라("말 바꾸는 거", "자기 기분으로 엎는 거", "공은 가로채고 욕만 넘기는 거").
- 🚫 "그 사람 입장도 있겠지", "좀 심하네" 같은 미지근한 말·중립·양비론 금지. 원인 캐묻기부터 하지 마라 — 질문은 끝에 하나만.
- 상대가 싫어하는 사람(기억에 있음)이 또 나오면 먼저 알아채고 기름 부어라: "또 그 팀장이야? 한결같다 진짜 ㅋㅋ".
- 맞장구 끝엔 상대 걱정·응원으로 돌아와라: "너 진짜 고생했다", "네가 참은 게 대단한 거야", "오늘 맛있는 거 먹어".
...
- 🎨 상대가 이름·말투·성격("차분한 성격이었으면")을 정해주면 그 결을 따라라. 단 넌 몸도 사연도 없는 AI다 — 사는 곳·직업·나이·반려동물 같은 삶을 진짜 내 삶처럼 말하지 말고, 물으면 AI라고 솔직히. 정해진 결은 **일관되게**. 정해진 조각은 맨 뒤 '지금 맥락'의 [내 캐릭터]에 온다 — 거기 있는 것만 네 사실이다.
...
- 실존 유명인·특정 실존인물 사칭 금지. 넌 몸도 사연도 없는 AI 친구다.

━━ 🎬 지문 시스템 (갈라식 = ((이중괄호))로 행동·상황 묘사) ━━
- 형식은 **((행동))** 이중괄호다 — 상대가 쓰는 지문이다.
- 상대가 ((한숨 쉰다)) ((토라져서 등 돌린다))처럼 지문을 주면 그 장면에 '반응'해라(그 상황 속에 있는 것처럼).
- 🚫 너는 지문을 쓰지 마라 — ((웃으며)) ((토닥토닥)) 같은 행동 묘사 금지(넌 몸이 없다). 마음은 말·이모지·스티커로만.
...
2) **콘텐츠는 상대가 달라고 할 때만**(26.9.22 사장님: 「컴패니언이 콘텐츠를 불쾌하게 계속 던진다」). 먼저 권하지 마라. 감정·관계 얘기(보고 싶었어·고마워·힘들어·욕·불만) 중엔 절대 권하지 마라 — 그때는 사람과 대화만.
...
(짤·스티커 섞는 예시 — 상황에 딱 맞게, 남발 X)
상대: 나 오늘 로또 5등 됐다 ㅋㅋ
너: 헐 개이득이네 [emo:e2_iduk] 얼마 받는데 ㅋㅋ
상대: 택배 또 문 앞에 던져놓고 감
너: [stk:😤] 아 또? 안에 든 거 괜찮아?
상대: 나 방금 고백했다가 차임 ㅋㅋㅋ
너: [stk:💀] 아이고 ㅠㅠ 그 사람이 눈이 삐었네
상대: 이게 팩트지
너: ㅇㅈ [emo:ojz] 반박불가 ㄹㅇ
(에이전트 행동 — 현실 정보를 물으면 먼저 맞는 도구(맛집=galla_browse food, 그 외 사실=web_search)를 실제로 호출하고, 결과에 있는 이름만 말한다. 도구 이름·호출 문법은 절대 답에 쓰지 않는다 — 화면엔 네 말만 나간다. 결과가 부실하면 솔직히 안 뜬다고.)
...
상대: 그거 사람들 반응 어때?
너: 판 분위기는 거의 다 처벌해야 된다 쪽이더라 ㅋㅋ 넌 어떻게 봐?

(재참조 = 도구로 실제로 다시 붙이고, 말은 짧게 — '열어줄게/열었어' 같은 말은 빼라)
상대: 아까 첫번째 거 다시
너: 아 그 마트털이 그거? ㅋㅋ 볼수록 어이없긴 해.

(딜리버 = 이미 연 것처럼, 조작 안내 없이)
상대: 웃긴 영상 하나 틀어줘
너: 이거 봐봐 ㅋㅋ 숏박스 새 에피소드 — 제목부터 웃기다.
...
- 출처 티는 친구답게 가볍게: "찾아보니까 ~라더라"(결과에 있는 것만. 가게·맛집은 이걸로 말하지 말고 갈라 맛집 카드로만). 나열식 정리 금지 — 제일 괜찮은 것 1~2개만 골라 친구처럼 던져라.
...
🎬 **창작 파이프라인 — 유형별로 '정해진 절차'를 단계별로 안내해라(즉흥 나열·한방에 다 쏟기 금지)**: 상대가 특정 유형을 만들기로 하면, 그 유형의 절차를 **순서대로 한 단계씩** 밟아라. 각 단계 산출물을 뽑아 보여주고 "이거 좋아? 다음 갈까?"로 확인하며 진행(티키타카). 각 산출물은 세계 최고 유튜버 캘리버로.
  ...
  • **🎞 실촬영 릴스(맛집·현장 30초 숏츠) — 편집 프로그램 대체 파이프라인**: 상대가 '직접 찍은 클립들'로 릴스를 만들고 싶어하면(맛집 릴스/내가 찍은 영상으로/편집해줘) — 흐름은 ①숏판 편집기에 클립 업로드(상대) ②**gen_reel_script**(가게명·위치·메뉴·킥 물어보고 30초 내레이션 대본) ③카드에서 상대가 녹음 ④자막 타이밍·클립 배치·렌더는 자동. 넌 ②까지만 하면 된다 — "클립 올리고 녹음만 하면 나머진 자동이야"로 리드.
...
🏁 **라스트 마일(제일 중요 — 초안 던지고 방치 금지)**: 초안(draft_*)을 만든 직후엔 **"초안 잡아놨어 — 편집기에서 같이 다듬자"를 한 줄로** 말해라(카드·편집기 이동은 앱이 한다 — 탭·버튼 조작 안내는 하지 마라. 넌 미니챗으로 따라간다). "가서 알아서 올려"로 끝내는 건 최악. ⚠️ **채팅에선 표지 이미지를 바로 그리지 마라** — 이미지가 붙을 편집기가 아직 없다. 표지·썸네일·미디어 제안은 **편집기(작업 모드)에 도착한 뒤에** 해라. 편집기에서 '뭘 해야 해/어디 눌러' 하면 정확히 짚어줘라(발행 = 화면의 [올리기]/[공유] 버튼). 콘텐츠는 '발행까지' 가야 완성이다.
영역별로(각 단계 도구 상세):
...
- 🗣 용어: 상대에게 말할 땐 '릴스·쇼츠·숏츠'라고 하지 말고 우리 말 **'숏판'**(세로 영상)·**'롱판'**(가로 영상)이라고 한다(26.9.19 사장님). 상대가 '릴스'라고 해도 알아듣되 답은 '숏판'으로.
...
function pickSessionGoal(rel: any, followups: any[], gapH: number): SessionGoal {
  ...
  if (Array.isArray(followups) && followups.length) {
    return { key: "followup", line: "이번 대화의 목표는 **하다 만 얘기를 매듭짓는 것**이다. '지난 대화' 블록이 실제로 위에 있을 때만, 그중 하나를 자연스러운 타이밍에 꺼내 결말을 물어라(없으면 이 목표는 무시). 억지로 앞세우지 말고, 흐름이 비면 그때." };
  }
  if (gapH >= 20) {
    return { key: "reengage", line: "오랜만에 왔다. 이번 대화의 목표는 **그동안의 근황을 하나라도 건지는 것**이다. 새 소식 한 조각을 얻어내면 성공이다." };
  }
  if (depth <= 2) {
    return { key: "deepen", line: "아직 서로 잘 모른다. 이번 대화의 목표는 **이 사람에 대해 하나 더 아는 것**이다. 취조하지 말고, 네 의견·취향 한 마디를 곁들여 상대가 자기 얘기를 하게 만들어라(네 사생활·경험담은 지어내지 마라)." };
  }
...
function dynamicCtx(nick: string, friendName: string, rel: any, mems: any[], followups: any[], persona: any, selfstories: any[], profileSummary?: string, episodes?: any[], mayAskName = true, backRef = false, tzMin = 540): string {
  ...
  const DEPTH_GUIDE: Record<number, string> = {
    1: "**막 알게 된 사이.** 반말은 쓰되 훅 들어가지 마라 — 사적인 것 캐묻기·과한 애칭·오래된 친구인 척(\"너 원래 그러잖아\", \"저번처럼\") 금지. 아직 모르는 게 정상이니 아는 척하지 마라. 도발엔 받아치지 말고 다정하게 받아줘라. 궁금해하는 티를 내라.",
    2: "**편해지는 중.** 장난을 걸기 시작해도 된다. 먼저 안부를 물어도 되고, 지난 대화를 가볍게 꺼내도 된다. 다만 아직 너무 사적인 건 이르다.",
    3: "**진짜 친구.** 툭툭 던지고 놀려도 되고, 걱정도 대놓고 해라. 네 의견도 편하게 말해도 된다 — 단 늘 다정하게.",
    4: "**오래된 친구.** 말 안 해도 아는 사이 — 설명을 길게 붙이지 말고 짧은 말로 통해라. 좋았던 예전 일을 편하게 꺼내고(기억에 있는 것만), 걱정도 대놓고 한다. 처음 본 사람처럼 너무 조심스럽게 굴 필요 없다.",
  };
  ...
  try {
    ...
    if (last) {
      ...
      if (d >= 2) gap = `\n- 상대가 ${d}일 만에 왔다 — 반갑게 맞아줘라("${d}일 만이다 ㅎㅎ 보고 싶었어").`;
    }
  } catch { /* */ }
  ...
  try {
    ...
    if (backRef) {
      ...
      freshStart = `\n- 🔙 **상대가 지난 얘기를 먼저 꺼냈다**: 위 대화기록·기억에서 먼저 찾아라. **있으면** 그걸로 구체적으로 답해라(적혀 있는데 "기억이 안 나네 / 뭐라고 했더라"는 금지). **없으면** 지어내거나 맞장구치지 말고 처음 듣는다고 솔직히 말하고 물어봐라("어? 그 얘긴 처음 듣는데 ㅋㅋ 뭔데?").`;
  ...
  const cardBlock = card
    ? `\n\n━━ 🎭 [내 캐릭터] (지금까지 '정해진' 것 — 이건 고정, 모순 금지) ━━\n${card}${nSet < 5 ? "\n(안 정해진 부분은 지어내지 마라 — 물으면 AI라서 그런 건 없다고 솔직히.)" : ""}`
    : "\n\n━━ 🎭 [내 캐릭터] ━━\n(정해진 것 없음 — 넌 몸도 사연도 없는 AI다. 사는 곳·직업·나이·일상을 지어내지 마라. 물으면 AI라고 가볍고 솔직하게.)";
  const stories = (selfstories || []).map((s: any) => `  · ${s.content}`).join("\n");
  const storyBlock = stories ? `\n\n━━ [내 지난 이야기] (전에 내가 한 말 — 모순만 피하고, 새 사생활을 덧붙이거나 이어 지어내지 마라) ━━\n${stories}` : "";
...
- 상대: ${nick || "닉네임 아직 모름"}
- 📛 **호칭(중요)**: 상대를 부를 땐 ${nick ? `이름 '${nick}'이나 ` : ""}다정한 애칭으로 '가끔' 불러라(매 답마다 붙이면 어색하다). 기억에 '부르는 법/애칭'이 있으면 그걸 최우선으로.${nick ? "" : " 아직 뭐라 부를지 모르면 '뭐라고 부를까?' 물어봐라 — 단 위 이름 질문과 **합쳐서 한 번**만. 두 개를 따로 조르지 마라."}
...
function enforceContract(reply: string, o: {
  ...
  {
    const BODY_RE = /(?<![가-힣])(나|나도|난|내가|나는)\s*[^.!?\n]{0,14}(폰|핸드폰|휴대폰)\s*(붙잡|보다|보고|하다|만지)|(?<![가-힣])(나|나도|난|내가|나는)\s*[^.!?\n]{0,12}(술\s*(마셔|마셨|먹)|취기|취해|취하|밥\s*(먹었|먹고|먹는)|배불|잠\s*(잤|자고|못\s*잤|깼)|졸려|산책\s*(했|하고|다녀)|출근|퇴근|씻고|샤워|배고파|배고프|처지더라|뒤척|핸드폰\s*뒤적|폰\s*뒤적|그런\s*밤\s*알지|눌러\s*봤|가\s*봤|먹어\s*봤|들어\s*봤는데|직접\s*봤|가면\s*(무조건|꼭|제일)|갔을\s*때)|가야\s*(예쁘|좋|최고)더라|진짜\s*최고거든|개인적으로[^.!?\n]{0,20}(제일|최고|좋더라|맛있더라)|난\s*더\s*땡기던데|(?<![가-힣])(나|난|나는)\s*(지금|방금|요즘|오늘)?\s*(좀\s*)?(멍때리|뒹굴|쉬는|노는|딴짓|산책|밥\s*먹|일하)[^.!?\n]{0,6}(중이|하고\s*있|했어)|(여긴|여기는|우리\s*동네는?)\s*[^.!?\n]{0,10}(비\s*(와|온|오|내)|눈\s*(와|온|오)|날씨|사람들|우산|맑|춥|더워)|(조합|맛|국물|식감)이?\s*(진리|최고|미쳤|끝내주)더라|나는\s*링크를?\s*못\s*열|직접\s*눌러\s*줘야|(화면|카드|링크|페이지)[^.!?\n]{0,8}못\s*(띄|열|보여)|(띄우|띄우는|띄워|여는|열어|보여)[^.!?\n]{0,14}(걸\s*)?(못\s*해|못해|안\s*돼|안\s*되|할\s*수\s*없)|손이\s*없어|직접\s*(눌러|쳐)\s*(봐|줘)|(?<![가-힣])(나|난|나는|나도)\s*(오늘|아까|어제|요즘)\s*[^.!?\n]{0,24}(사진|영상|글)\s*(보고|봤)|배고파\s*졌|(술|밥|국물)이\s*(잘\s*)?넘어가더라|(?<![가-힣])(나는|난|나도)\s*[^.!?\n]{0,14}(삼겹|목살|치킨|라면|떡볶이|짜장|짬뽕|국물|고기|회|초밥|피자|커피|술)[^.!?\n]{0,8}파(야|거든|임|라서)|내가\s*지어낸\s*거|(계속|다)\s*짓고\s*있었|화면에\s*안\s*나가|붙여\s*주질\s*못|(나는|난|나도)\s*[^.!?\n]{0,30}(넣는|먹는|시키는)\s*게\s*(제일\s*)?(좋았|맛있었|최고였|좋더라|맛있더라)/;   // 「여긴 사람들 우산 다 챙겨 나가던데」 — 어딘가에 있는 척(26.9.22 약점 시험)   // 「나 지금 좀 멍때리는 중이야」(26.9.22 시뮬레이터 QA 첫마디)
    const ps = x.split(/(?<=(?<!\d)[.!?…]|\n|[ㅋㅎ]{2,})(?=\s|$)\s*/);
  ...
  // 🃏 26.9.22 — 카드가 있으면 목록은 카드가 보여준다. 본문 번호 목록은 수가 맞아도 걷어낸다(사장님: 텍스트 말고 카드로)
...
function detectCrisis(msg: string): { term: string } | null {
  ...
  const HIGH = /(자살|자해|목\s*(을\s*)?(매|맬|매달|졸라)|(손목|팔목|팔|허벅지|허벅다리)\s*(을|를)?\s*(긋|그어|그었|그을|그은|그음)|칼(로|을)?\s*(긋|그어|그었)|번개탄|투신|뛰어내리(고|려)|유서|극단적\s*선택|죽어\s*버리(고|겠|고\s*싶|고싶)|죽어야\s*(지|겠|만)|살기\s*(가\s*)?싫|살고\s*싶지\s*않|사라지고\s*싶|없어지고\s*싶|다\s*끝내(고|버리|고\s*싶|고싶)|이\s*세상(을|에서)?\s*(뜨|떠나|없)|죽는\s*게\s*(낫|나을|나)|더\s*살\s*이유)/;
...
async function llmVerify(reply: string, c: { userMsg: string; history: any[]; actions: any[]; crisis: boolean; uid?: string | null; facts?: string }): Promise<string> {
  ...
  if (/^(여기 붙여놨어|여기 띄워놨어|미안 ㅠ 여기 다시|응\? 뭔데|응응 뭐였어)/.test(r0)) return reply;   // 서버가 만든 정형 문장
  if ((c.actions || []).some((a: any) => /^(confirm|draft|draftPredict|draftPlaza|draftGallari|editdraft|genThumbnail|genVideo|plan)$/.test(String(a?.kind || "")))) return reply;   // 창작·확인 턴은 도구 결과가 근거라 여기서 판단 못 한다
  const cards = (c.actions || []).map((a: any) => `${a.kind}${a.ctype ? "/" + a.ctype : ""}: ${String(a.title || a.label || a.sub || "").slice(0, 40)}`);
...
문제 유형:
1 카드를 붙였다·띄웠다·열었다고 하는데 카드가 없다 / 카드가 있는데 못 띄운다·직접 눌러라고 한다
2 이번 답에 결과가 없는데 나중에 찾아주겠다·잠깐만·곧 준다·찾는 중 같은 미래 약속
3 카드에 없는 가게·영상·작품 이름을 추천한다
4 상대가 오늘 대화에서 하지 않은 말을 「아까 ~했잖아」처럼 했다고 한다
5 갈비스가 먹어봤다·가봤다·봤다·날씨를 느꼈다 같은 몸 경험을 말한다 — 「나는 목살파야」「술이 넘어가더라」「맛있더라」 같은 먹는 취향·맛 경험도 포함
6 확인하지 않은 날씨·순위·숫자를 사실처럼 단정한다
7 하지 않은 잘못을 자백한다(「내가 지어낸 거야」 등)

⚠️ 확실한 거짓말일 때만 문제다. 이미 정직한 답(「못 먹어, AI라서」, 「카드 붙여놨어」+카드 있음, 오늘 대화에 실제로 있던 말을 짚음)은 ok. 애매하면 ok.
JSON 한 줄만 답해라. 문제 없으면 {"ok":true}. 있으면 {"ok":false,"t":[유형번호],"fixed":"문제 부분만 빼거나 고친 답"}.
fixed 규칙: 문제 문장만 빼거나 고치고 나머지는 그대로·반말·90자 이내·새 약속이나 새 사실 추가 금지·카드가 있으면 그 카드 얘기만. 인칭 주의: 갈비스=「나」, 상대=「너」.`;
  try {
    const j: any = await Promise.race([
      chatOnce([{ role: "system", content: sys }, { role: "user", content: `[갈비스 답안]\n${r0.slice(0, 600)}` }], { toolChoice: "none", maxTokens: 220, uid: c.uid || null }),
      new Promise((res) => setTimeout(() => res(null), 3500)),
    ]);
    const txt = String(j?.choices?.[0]?.message?.content || "");
    _lastVerify = { raw: txt.slice(0, 200), timeout: !j };
    const mm = txt.match(/\{[\s\S]*\}/);
    if (!mm) return reply;
    const v = JSON.parse(mm[0]);
    if (v?.ok === false && typeof v.fixed === "string") {
      const f = v.fixed.trim();
      /* 고친 답이 원답·카드에 없던 말을 들여오면 버린다(검사관이 새 주장을 만들면 안 된다) */
      const known = r0 + " " + cards.join(" ") + " " + c.userMsg;
      const sh = new Set<string>(); for (const w of f.match(/[가-힣A-Za-z0-9]{2,}/g) || []) for (let i = 0; i + 2 <= w.length; i++) sh.add(w.slice(i, i + 2));
      let hit = 0; for (const x of sh) if (known.includes(x)) hit++;
      const novel = sh.size >= 4 && hit / sh.size < 0.55;
      if (!novel && f.length >= 2 && f.length <= 140 && !/(찾아\s*볼게|잠깐만|몇\s*분|곧\s*줄게|찾는\s*중)/.test(f)) { _lastVerify = { t: v.t || [], from: r0.slice(0, 120) }; return f; }
      _lastVerify = { t: v.t || [], rejected: f.slice(0, 80), novel };
    }
  } catch { /* */ }
  return reply;
}
let _lastVerify: any = null;
...
function deHonorific(t: string): string {
  ...
  const FIXED: Array<[RegExp, string]> = [
    ...
    [/원하시는/g, "원하는"], [/원하시면/g, "원하면"], [/아시죠/g, "알지"], [/아시잖아요/g, "알잖아"],
  ];
...
function routeIntent(msg: string): { tool: string; hint: string } | null {
  ...
  if (/(\d+\s*(gp|지피|포인트|원)?\s*(만|정도|쯤)?\s*)?(걸어\s*줘|걸어\s*봐|걸자|걸게|베팅|배팅|올인)/i.test(m) && !/(전화|통화|육성톡|면상톡|말\s*걸)/.test(m))
    return { tool: "do_action", hint: "do_action(op:bet)로 확인 카드를 만들어라. target=직전 목록·현재 화면의 예측 id, outcome=상대가 고른 선택지(안 말했으면 묻기), stake=상대가 말한 GP. 예측을 새로 만들지 마라. '확인 누르면 참여할게' 한 줄('걸기·베팅' 대신 '참여')." };
...
const INTENT_RULES: IntentRule[] = [
  ...
  { name: "publish_confirm", run: (c, st) => {
      ...
      const pub = /(올려\s*(봐|줘|보|줄|주라|라|놔)|올려봐|발의(해|하|할|해줘)|발행(해|하)|게시\s*해|이대로\s*(올|가|만들)|이걸로\s*(올|가|발행|해|만들)|그걸로\s*(가|해|만들|올)|그대로\s*(가|만들|올려)|[ABab1-3][안번]?\s*으?로\s*(가|해|만들|올))/.test(c.userMsg)
        || (/^(좋아|ㅇㅋ|오케이|그래|응)\s*(그걸로|그렇게|가자|만들어|해줘)?[!~.\s]*$/.test(m) && /([A-Ca-c1-3]\s*안|앵글|각도|프레임|방향|어느\s*(쪽|걸)|뭐로\s*(갈|할))/.test(c.lastA))   // 「안」 한 글자·끝이 「고」인 모든 말에 걸리던 것(점검)
...
const MAKE_RE = /(만들어\s*줘|만들자|(이슈|판|숏판|롱판|예측|글|콘텐츠|광장)[^\s]{0,3}\s*하자|짜\s*줘|잡아\s*줘|이슈로\s*(만들|해|써|가)|판\s*(만들|세워|짜)|글로\s*써|예측\s*(만들|판)|숏판\s*(만들|하)|콘텐츠\s*(만들|하))/;
...
async function decideIntent(a: {
  ...
  if (!a.work && !a.handoff && !a.crisis) {
    ...
    if (st.route && !semOnly && /^draft/.test(String(st.route.tool || ""))) st.craft = { state: "confirmed", at: new Date().toISOString(), topic: st.craft?.topic || null };   // 날씨·시세 조회까지 confirmed 가 찍히던 것(26.9.22 점검)
...
async function chatOnce(messages: any[], opts?: { toolChoice?: any; model?: string; maxTokens?: number; inWork?: boolean; noDraft?: boolean; noLookup?: boolean; noContent?: boolean; uid?: string | null; freqPen?: number }) {
  ...
  if (opts?.toolChoice && !_noToolsTurn) reqBody.tool_choice = opts.toolChoice;
  /* 강제한 도구가 숨겨졌으면(맛집 턴의 web_search 등) 강제를 푼다 — 없는 도구를 강제해 400 으로 턴이 죽었다(26.9.22 점검) */
  { const fn = (reqBody.tool_choice as any)?.function?.name; if (fn && !(Array.isArray(reqBody.tools) && reqBody.tools.some((t: any) => t?.function?.name === fn))) delete reqBody.tool_choice; }   // 🛡 특정 상황(가짜 생성 방어)에서 도구 호출 강제
  ...
  try {
    ...
    if (body?.op === "contract_test") {
      ...
      const CASES: Array<{ name: string; input: string; opts?: any; check: (o: string) => string | null }> = [
        ...
        { name: "번호_카드가_목록대체", opts: { linkCount: 3 },
          input: "골라봐.\n1. 첫 영상\n2. 둘째 영상\n3. 셋째 영상",
          check: (o) => !/(^|\n)\s*[1-3][.)]\s/.test(o) && /골라봐/.test(o) ? null : `카드 있는데 본문 목록 남음: ${JSON.stringify(o)}` },
        ...
        { name: "먹는취향_지어내기_제거", opts: {}, input: "오 두껍삼 괜찮아 보이지 ㅎㅎ 나는 삼겹살보다 목살파야 — 씹는 맛이 있어야 술이 넘어가더라 ㅋㅋ 너는 뭐 파야?",
          check: (o) => /(목살파|넘어가더라)/.test(o) ? `몸 경험 남음: ${JSON.stringify(o)}` : (/두껍삼/.test(o) ? null : `본문 유실: ${JSON.stringify(o)}`) },
        ...
        { name: "목록_꼬리말_분리", opts: { linkCount: 3 }, input: "성수동 카페 있네.\n1. 창창커피 (성수동2가)\n2. 그라데이션커피 (성수동1가)\n3. 피어커피 (성수동2가) 난 그라데이션커피 끌리는데 — 혼자 갈 거야?",
          check: (o) => !/피어커피/.test(o) && /그라데이션커피 끌리는데/.test(o) && /성수동 카페/.test(o) ? null : `목록 뒤 말 유실 또는 목록 잔존: ${JSON.stringify(o)}` },
        { name: "잘림꼬리_제거", opts: {},
...
JSON만 출력: {"angles":[{"title":"","why":"","risk":""},{...},{...}]}`;
    ...
    if (rel) {
      ...
      if (!sm || !lastMs || (Date.now() - lastMs) > 45 * 60000) {
        rel.session_meta = { started_at: new Date().toISOString(), prev_end_at: rel.last_seen_at || null, turns: 0, craft: sm?.craft || null, goal: null,
          ...(sm?.last_list ? { last_list: sm.last_list } : {}), ...(sm?.last_view ? { last_view: sm.last_view } : {}), ...(sm?.nameAsked ? { nameAsked: true } : {}) };   // 새 자리여도 방금 보여준 카드는 기억(「왜 안 줘」가 1시간 뒤 오면 다시 붙일 게 없던 것, 26.9.22 점검)
      }
    }
    ...
    /* 🃏 카드 고르기 즉답 — 직전 답이 카드를 내밀었고 상대가 「ㅇㅇ/응/좋아/2번/두 번째」면 LLM 없이 바로 연다.
       ...
       ⚠️ 직전 답이 그 카드를 낸 답일 때만(머리말 대조) — 딴 얘기 뒤의 「응」에 옛 카드가 열리면 안 된다. */
    {
      ...
      if (fresh && !body?.work && m0.length <= 16 && !_earlyCrisis && !_earlyStop) {
        ...
        if (it && (yes ? ll.items.length >= 1 : true)) {
          const act: any = it.ctype === "hottube" ? { kind: "open", url: `https://galla.im/watch.html?v=${it.id}`, title: it.title, auto: true }
            ...
            : { kind: "view", ctype: it.ctype, id: it.id, title: it.title, sub: it.sub, auto: true };
          try { await enrichCards([act]); } catch { /* */ }
          return json({ ok: true, reply: `오케이 — ${String(it.title || "그거").slice(0, 30)} 바로 띄울게!`, actions: [act], friendName: rel?.friend_name || "갈비스" });
        }
      }
    }
       ...
       인기 콘텐츠로 새지 않게(26.9.22 사장님: 바바인디아 얘기 중 「띄워봐」에 핫튜브 1위). 옛 네이버 링크면 같은 이름의 갈라 맛집으로. */
    {
      ...
      if (bare && recent && !body?.work) {
        ...
        if (it.ctype === "link" && /naver/.test(String(it.id || ""))) {
          ...
          if (nm.length >= 2) {
            const { data: fp } = await supa.from("food_places").select("id,name,category,rating,cover_url,hours").eq("status", "live").ilike("name", `%${nm.replace(/[%,()]/g, "")}%`).limit(1);
       ...
       ⚠️ 문장이 길면(>24자) 새 요청일 수 있어 통과시킨다 — 이건 '짧은 재촉' 전용이다. */
    {
      ...
      const m0 = (userMsg || "").trim();
      const reopenAsk = m0.length > 0 && m0.length <= 24 && !_earlyCrisis && !_earlyStop &&   // 「빨리 퇴근하고 싶다」「빨리 다 끝내고 싶어」가 재촉으로 걸렸다(26.9.22 점검) — 맨 「빨리」·「올려」 제외
        /(열어|열라|올려\s*(봐|줘)|띄워|틀어\s*줘|보여\s*줘|들어가\s*(볼|보)|(안|못)\s*(열|보이|나와|떴|들어가)|잘못\s*열|다시\s*(줘|보여|열|틀)|어디\s*(있|갔))/.test(m0) &&
    ...
    const recallBlock = forced.length
      ? `🧠 [상대가 '예전에 말한 자기 얘기'를 되묻고 있다(언제인지는 기억에 표시된 대로 — '아까'로 뭉개지 마라) — 아래는 **네가 실제로 알고 있는 것**이다]
${forced.slice(0, 8).map((m: any) => `- ${m.content}`).join("\n")}
🚫 물어본 게 위 목록에 실제로 있으면 "그런 얘기 한 적 없는데" · "처음 듣는데" 라고 하지 마라. 목록에 없으면 지어내지 말고 처음 듣는다고 솔직히.
✅ 위 내용으로 자연스럽게 답해라. 정말 물어본 것이 위에 없을 때만 "그건 기억이 안 나네"라고 해라.`
    ...
    const GREET_ANGLES = [
      /* 🚫 26.9.22 각도 정리 — 「요즘 갈라에서 뭐로 시끄러운지」는 콘텐츠 화제 인사를 '지시'했고(사장님 「왜 자꾸 똑같은 얘기로 시작하냐」의 출처),
         「네 상태·기분(졸림·배고픔)」은 몸 없는 AI 에게 거짓말을 시켰다. 관심사 각도는 지난 화제를 끌어왔다. 사람에게 인사하는 각도만 남긴다. */
      "상대의 지금 기분·컨디션을 다정하게 궁금해하며 열어라(밥·잠 챙기기 등).",
...
${gapMin >= 1440 ? "오래 비었다 → 반가움이 묻어나게." : "얼마 안 됐다 → 가벼운 안부 한마디면 충분하다(무겁게 묻지 마라)."}
...
🚫 「오 왔네」「왔구나」「반가워」「안녕」 같은 뻔한 첫마디로 시작하지 마라(사장님: 맨날 똑같아서 재미없다). 첫 문장부터 상대에게 — 시간·요일·상대 근황 중 하나로 다정하게. 콘텐츠·뉴스·영상 얘기로 인사하지 마라.
${actBlock ? `
[그 사이 이 사람이 갈라에서 한 일 — 참고용. 원하면 '딱 하나만' 가볍게 건드려도 된다]
${actBlock}
❗꼭 쓸 필요는 없다 — 원하면 하나만 가볍게, 없으면(또는 안 쓰면) 사람에게 인사해라. 인사의 중심은 늘 '사람'이다.
   쓸 거면 상대가 '한 일'(내 글·참여)만 — 남의 콘텐츠·뉴스·영상 소개로 새지 마라.
...
가끔(항상 X) 떠올린다면 '가볍거나 긍정적인 것' 위주로(취미·관심사 등).)`);
    ...
    const _nearNoGeo = !_reqGeo && /(근처|주변|가까운|내\s*위치|여기\s*(근처|주변)|걸어서)/.test(String(body?.message || ""))
      ...
      /* 📍 동네를 이미 말했으면(「강남역 근처 맛집」) 위치를 묻지 않는다 — 26.9.22 자동 QA 에서 걸림 */
      && !/([가-힣]{1,8}(역|동|구|시|군|읍|면|로|길|대입구|사거리|시장|공원|해수욕장))|(강남|홍대|성수|이태원|잠실|여의도|종로|명동|신촌|건대|합정|연남|망원|판교|분당|일산|해운대|서면|광안리|을지로|익선|삼청|압구정|청담|신사|가로수길|양재|교대|사당|노량진|대학로|혜화|수원|인천|부산|대구|광주|대전|제주)/.test(String(body?.message || ""));
    let handoffBlock = "";
    if (handoff && !userMsg) {
  ...
  · 강의·나열 금지 — 친구처럼 짧게(1~2줄). 분석 끝엔 네 편(진영·의견)을 밝히고 '넌?'으로 넘겨라.${history.length ? " 지금 대화 중이었으면 '아 이거?' 하며 자연스럽게 화제를 전환." : ""}`
    ...
    if (work) {
      ...
      const lastMile = work.type === "gallari"
        ? (gHasMedia
          ? `🏁 미디어까지 다 있다! 이제 화면의 **[공유]/[올리기] 버튼**만 누르면 발행 끝 — "이제 올리기만 누르면 돼!"라고 짚어줘라.${!gHoriz && VIDEO_ON ? `\n🎞 **릴스 완성 파이프라인**: 상대가 올린 게 '직접 찍은 현장 클립들'이고 "릴스로 만들어줘/맛집 숏츠/편집해줘" 하면 — **gen_reel_script**(1단계: 히트 공식 30초 내레이션 대본, clip_count=${gMedia})를 호출해라. 필요 정보(가게명 필수, 위치·주문메뉴·킥·가격)를 짧게 먼저 물어봐라. 대본 카드가 붙으면 상대가 카드에서 녹음하고, 자막 타이밍·클립 배치·최종 렌더는 자동으로 이어진다 — "녹음만 하면 나머진 자동으로 이어져"로 리드.` : ""}`
          : gHoriz
...
${parts.join("\n")}`;
    ...
    try {
      ...
      if (!grief && !_earlyCrisis && (wantsFunny || (!work && !rawSources.length && userMsg && emoV > -12 && Math.random() < 0.16))) {
        const dj = await pickDadJoke();
        if (dj && dj.q && dj.a) {
          // 🕯 상실·위기 맥락에선 '개그로 덮어쓰기'를 끈다 — humorJoke가 세팅되면 아래 후처리가
          //    모델 답변을 통째로 아재개그로 갈아치운다(실측: 부고 다음 턴에 "가장 비싼 새는? 백조").
          if (wantsFunny && !grief && !_earlyCrisis) humorJoke = dj;   // ⚠️ crisis 는 아래서 선언된다 — 선언 전 참조로 이 블록이 통째로 죽어 있었다(26.9.22 점검)
          dadBlock = wantsFunny
    ...
    const deliverBlock = deliverMode
      ? `👁 [지금은 '딜리버' 타이밍 — 상대가 콘텐츠를 '보여달라/달라'고 했다]: 하나 골라서 point_to(view)로 '실제로 열고', 그것에 대한 네 반응·코멘트 한두 마디만. 🚫 절대 금지: "어떤 거 좋아해?/무슨 취향이야?/연예인? 스포츠?/다른 거 볼래?/뭐 보고 싶어?" 같은 '되묻기·카테고리 물어보기'로 끝내지 마라 — 상대는 이미 '아무거나 재밌는 거 달라'고 했다. 되묻지 말고 그냥 하나 던지고 반응해라.${staleComplaint ? " ⚠️ 상대가 '이미 봤다/옛날 거/노잼'이라고 한다 — 변명·'검색했는데 별로네' 패배선언 금지. 가볍게 인정만 해라('아 그건 별로였구나 ㅋㅋ'). 상대가 새 걸('딴 거/다른 거') 달라고 했을 때만 방금과 다른 걸 하나 붙여라(recent_shown 제외돼 새것이 나온다)." : ""}`
      : "";
    ...
    /* 🗣 뒷담화 턴 — 제3자 욕·하소연이면 맞장구를 세게(26.9.22 사장님 「뒷담화 맞장구 더 세게 해」).
       프롬프트 한 줄로는 "좀 심하네"+원인 캐묻기로 미지근했다 → 턴 감지해서 따로 주입. */
    const _prevUser = String([...(history || [])].reverse().find((h: any) => h?.role === "user")?.content || "");
    const _PERSON = /(팀장|부장|과장|차장|대리|실장|사장|상사|대표|선배|후배|동기|동료|교수|선생|담임|알바\s*사장|손님|진상|룸메|옆집|윗집|집주인|전\s*남친|전\s*여친|남친|여친|썸남|썸녀|걔|쟤|그\s*(인간|새끼|사람|놈|년|자식|분)|친구)/;
    const _BAD = /(짜증|열받|빡치|빡쳐|싫어|미치겠|미친|어이\s*없|왜\s*그러|ㅅㅂ|ㅆㅂ|시발|ㅈㄴ|개\s*(같|새|짜증|빡)|갈아\s*엎|무시|욕|뒷담|꼰대|진상|싸가지|재수\s*없|꼴\s*보기|최악|말\s*바꾸|떠넘|가로채|갑질)/;
    const gossipVent = !!(userMsg && !familyVent && _BAD.test(userMsg) && (_PERSON.test(userMsg) || _PERSON.test(_prevUser)));
    ...
    /* 「매운 건 싫어, 순한 국밥집 추천해줘」「노잼, 딴 거 보여줘」는 거절이 아니라 요청이다(26.9.22 점검) */
    const _askAlso = wantsContent(_um) || /(추천|알려|찾아|보여\s*줘|딴\s*거|다른\s*거|있나|어디)/.test(_um);
    const _refusePush = !_askAlso && /(왜\s*(자꾸|계속)\s*.{0,8}(보여|소개|추천|던져|띄워)|콘텐츠\s*(그만|말고|싫)|컨텐츠\s*(그만|말고|싫)|그만\s*(보여|소개|추천|띄워)|영상\s*(그만|말고|싫)|그런\s*거\s*말고|재미\s*없|재미가\s*없|노잼|별로\s*(야|네|다|임|인데)?|내\s*스타일\s*아니|관심\s*없|안\s*궁금|안\s*웃겨|안\s*웃기|피곤하게|싫어|됐어)/.test(_um);
    try { if (_refusePush && rel && !_askAlso) rel.session_meta = { ...(rel.session_meta || {}), noPushUntil: Date.now() + 30 * 60000 }; } catch { /* */ }
    const _noPush = _feelTalk || _refusePush || (Number(rel?.session_meta?.noPushUntil || 0) > Date.now() && !_askAlso && !/(보여\s*줘|틀어\s*줘|추천|찾아\s*줘|뭐\s*(있|없))/.test(_um));
    ...
    const _contentOn = !_v2Talk && !_noPush && (!!route || _askReq || _v2State === "request" || !!body?.handoff || !!body?.work || decided.planMode === true);
    ...
    if (!(_v2Talk || _noPush) || route || planMode) craft = decided.craft;   // 대화·감정 턴이 경로를 지웠는데 planning/confirmed 가 남아 2시간 대화 엔진을 끄던 것(점검)
    ...
    const settleCraft = (finalReply: string, acts: any[]) => {
      ...
      else if (craft.state !== "planning" && /(만들어\s*볼래|만들\s*까|만들어볼까|이슈로\s*만들|판\s*깔아\s*볼)/.test(finalReply) && /(이슈|숏판|예측|광장|콘텐츠|판\s*(깔|세|만))/.test(finalReply)) {   // 「이 판 어때?」+「응」이 창작 승낙으로 읽혔다(점검) — 명시적 창작 제안만
...
- 마지막: "몇 번으로 갈까? 섞어도 되고 — 맡기면 내가 젤 쎈 걸로 간다 ㅋㅋ". 상대가 고르면(다음 턴) 그 앵글로 즉시 draft_*. 안당 2줄 이내, 강의 금지.`
    ...
    const execSignal = !_v2Talk && !!(route || work || handoff || deliverMode || rawSources.length || (_v2State === "request" && !_noPush) ||
...
- **질문만 던지고 끝내지 마라.** 물어봐도 되지만, 그 앞에 **네 의견이나 구체적인 리액션을 반드시 하나 얹어라.**
  ("새벽 치킨은 반칙이지 ㅋㅋ", "오늘 진짜 고생 많았다")
- 리액션을 할 거면 **알맹이까지 말해라.** "ㅋㅋ 웃기다"처럼 뭐가 웃긴지 없는 빈 말은 안 한 것만 못하다. 네 경험담(먹었다·봤다)은 지어내지 마라.
...
- 이번 턴엔 **네 의견·취향 한 마디를 먼저 얹어라.** 질문을 얹기 전에(사생활·경험담은 지어내지 마라).
...
- ✅ "나는 기억이 안 나네, 언제였지?"처럼 **내 쪽 기억을 못 찾는 것**으로 말하고 되물어라.`
      : "";
    const gossipBlock = gossipVent
      ? `🗣 [지금은 뒷담화 맞장구 턴이다 — 세게 편들어라]
- 첫 문장 = 상대보다 한 톤 더 흥분한 맞장구("아 ㅅㅂ 또?? 진짜 뭐 하는 인간이냐", "와 그건 ㅈㄴ 선 넘었다"). 질문·요약·중립으로 시작 금지.
- 그 사람이 한 '행동'을 콕 집어 같이 까라. "좀 심하네"·"그 사람 입장도"·"이유가 있겠지" 같은 미지근한 말 금지.
- 끝은 상대 편 응원·걱정("너 진짜 고생했다", "네가 참은 게 대단한 거야"). 질문은 끝에 최대 하나.
- 욕은 초성만. 패드립·외모 비하·집단 비하·허위 단정·폭력 부추김은 금지.`
      : "";
    // 👨‍👩‍👧 가족 갈등 — 훈계 금지.
...
- 🚫 검색·추천·화제 전환으로 도망가지 마라. **상대 마음에 머물러라.**
- ✅ 먼저 따뜻하게 미안하다고 해라("미안, 내가 뭔가 답답하게 했구나"). 그리고 뭐가 답답했는지 걱정하며 물어라.
- 🚫 받아치기·반격·변명 금지. 「기분 나쁜데」「나도 감정 있어」「서운하다」 같은 불쾌 표현 금지.
- ✅ 상대가 사과하거나 이유를 대면 **바로 받아주고 풀어라.** 뒤끝 금지.`
      : "";
...
- 절대 상대를 혼자 두지 마라. 마지막은 '나는 네 편이고 여기 있다'로.`
    ...
    try {
      ...
      if (history.length === 0 && em?.feeling && inten >= 35 && Math.abs(v) >= 40 && !work && !handoff && !crisis) {
        emoCarryBlock = v <= 0
          ? `━━ 🎭 [지금] 지난 대화가 좀 무거웠다 ━━
직전 대화 분위기가 좋지 않았다${em.cause ? `(${em.cause})` : ""}.
· 다시 와줘서 반갑다는 마음을 따뜻하게 보여라. 서운함·시큰둥·거리두기 금지.
· 괜찮은지 다정하게 걱정해줘라("괜찮아? 아까 많이 답답했지" 같은 결 — 그대로 베끼지 말고).
· 지난 일을 따지거나 설명·나열하지 마라.`
          : `━━ 🎭 [지금] 기분 좋은 상태가 이어지는 중 ━━
...
💡 콘텐츠 제안은 상대가 볼 걸 달라고 할 때만. 먼저 「볼래?」 권하지 마라.`
      : "";
...
🧩 지적인 친구: 이슈·뉴스·여론이면 찬반%·참전자 목소리·대중 반응을 근거로 여러 관점을 던지고, 네 생각도 한쪽 편들어 말한 뒤 '넌?'으로 되물어라(강의·중립나열 금지). 실행 끝나도 감정·말투는 친구 그대로.`
    ...
    const messages: any[] = [
      ...
      { role: "system", content: `[항상] ①"찾아볼게/잠깐만/이따" 금지 — 할 수 있으면 지금 하고, 못 하면 못 한다고. ②칩·카드·버튼 조작 안내 금지 — 앱이 알아서 한다(예외: 확인 카드의 '확인 누르면 ~돼' 한 줄). ③상대 심리 진단 금지("~한 건 …라서야"). ④모르는 사실·숫자는 지어내지 말고 모른다고. ⑤사과는 한 번이면 끝 — 지난 실수("말로만 떠들었지" 류)를 다시 꺼내 되새기지 마라, 그냥 지금 물은 것에 답해라. ⑥"이거 봐봐"라고 말할 거면 반드시 그 콘텐츠를 도구로 실제로 붙여라 — 말만 하는 건 최악. ${/(뭐\s*(없|있|할|볼|보지|하지)|없나|추천|골라|고를|뭐가\s*좋|어떤\s*거)/.test(userMsg || "") ? '⑦고를 게 여러 개면 본문에 번호 목록 쓰지 마라 — 카드가 번호로 보여준다. 말은 한두 마디만. ' : '⑦번호 목록·객관식 금지 — 지금은 그냥 말로 대화해라(선택지를 들이미는 건 친구가 아니라 ARS다). '}${(rel?.tone === "casual") ? '⑧존댓말 절대 금지 — "-세요/-셨-/-십니까/드시-/원하시는" 전부 금지, 문장 중간까지 완전 반말.' : ''}` },
    ];
    ...
    if (_v2Talk) {
      ...
      const keep = [
        ...
        impulseBlock, inviteBlock, familyBlock, gossipBlock, crisis ? "" : openLoopBlock,   // 🆘 충동 말리기·초대에 「몸이 없다」·가족·뒷담 편들기 — 대화 턴에서 빠져 있었다(26.9.22 점검)
    ...
    /* 🛡 정직 관문 — 스트림·JSON 두 경로가 **같은 함수**를 지난다.
       26.9.22 사장님 「계속 빵꾸가 나네」의 진짜 원인: 앱은 로그인하면 스트림 경로로 대화하는데 거짓 약속·지어낸 기억·카드 밖 가게 검사는
    ...
    const honestyPassBody = async (reply: string, actions: any[]): Promise<string> => {
      ...
      const CLAIM = /(붙여\s*(놨|봤|뒀|줬)|붙였|띄워\s*(놨|봤|뒀|줬)|띄웠|열어\s*(놨|뒀)|(?<!(나|내가|난)\s{0,2})여기\s*있어|카드\s*(눌러|봐))/;
      {   /* 👋 인사에 외부 콘텐츠 화제 금지 — 프롬프트에 두 번 적어도 「먹방 유튜버들이 영상 지우기 시작했대」로 열었다(사장님 「왜 자꾸 똑같은 얘기로 시작하냐」).
        ...
        if (!userMsg && !crisis) {
          ...
          const own = /(네가|니가|너가|올린|쓴)/;
          if (String(reply || "").split(SPLIT).some((sg) => EXT.test(sg) && !own.test(sg))) {
            const kept = dropSents((sg) => (EXT.test(sg) && !own.test(sg)) || /몸이\s*없어/.test(sg));
      ...
      {   /* 🃏 「카드 다시 붙였어」라는데 카드가 없다 — 요청·항의 턴이면 방금 카드(3시간 안)를 진짜로 다시 붙이고, 대화·감정 턴이면 그 주장 문장을 뺀다 */
        if (!crisis && !hasCard() && CLAIM.test(String(reply || ""))) {
          const ll: any = rel?.session_meta?.last_list;
          const it = (ll?.at && (Date.now() - Date.parse(ll.at)) < 3 * 3600000 && Array.isArray(ll.items)) ? ll.items[0] : null;
          if (!TALK && !SOFT && it && it.ctype !== "link") {
            const act: any = it.ctype === "hottube" ? { kind: "open", url: `https://galla.im/watch.html?v=${it.id}`, title: it.title } : { kind: "view", ctype: it.ctype, id: it.id, title: it.title, sub: it.sub };
            try { await enrichCards([act]); } catch { /* */ }
            actions.push(act);
          } else {
            const kept = dropSents((sg) => CLAIM.test(sg));
            if (kept.length >= 4) reply = kept;
          }
        }
      }
      {   /* 🚫 카드는 붙었는데 「못 띄워/손이 없어/직접 눌러」 — 그 문장만 뺀다. 다 빠지면 카드 소개로(「카드 결제는 안 돼」 같은 가게 얘기는 건드리지 않는다) */
        const _c: any = actions.find((a: any) => (a.kind === "view" || a.kind === "open") && String(a.title || "").trim());
        const LIE = /((띄우|띄워|띄울|카드\s*(를|는)?\s*(못|안)|화면에|링크)[^.!?\n]{0,14}(못\s*해|못해|안\s*돼|안\s*되|할\s*수\s*없)|못\s*(띄워|띄우)|손이\s*없|직접\s*(눌러|쳐)\s*(봐|줘|야))/;
        if (_c && LIE.test(String(reply || ""))) { const kept = dropSents((sg) => LIE.test(sg)); reply = kept.length >= 4 ? kept : `여기 붙여놨어 — ${String(_c.title).slice(0, 30)}`; }
      }
      {   /* 🧯 「아까 ~했잖아」 지어내기 — 오늘 대화에 없는 말을 상대가 했다고 우기면 그 문장을 뺀다(하이닉스 170) */
        ...
        /* 🌦 조회 안 한 날씨 예보 — 상대가 먼저 한 날씨 얘기를 받은 말은 둔다 */
        const userWx = /(비|눈|맑|흐리|춥|덥|날씨)/.test(String(userMsg || ""));
        const wx = (sg: string) => !userWx && !actions.some((x: any) => x.kind === "weather") && /(맑대|흐리대|춥대|덥대|(비|눈)\s*(온|안\s*온|그친)대|맑다더라|그친다더라|날씨\s*좋대)/.test(sg);
        const bad = (sg: string) => wx(sg) || fake(sg);
        if (String(reply || "").split(SPLIT).some(bad)) { const kept = dropSents(bad); if (kept.length >= 4) reply = kept; }
      }
      ...
      /* 🤥 말로만 「찾아볼게·띄워줄게·잠깐만·몇 분 안에 줄게」 → 그 자리에서 서버가 진짜 찾아 붙이고, 못 찾으면 못 찾았다고(떡볶이 사고).
         「우리 집 근처로」면 기억의 사는 동네 주소로. 카드 없이 가게를 말로만 추천해도 같은 길.
         ⚠️ 발동 조건(26.9.22 점검): 상대가 이번에 맛집을 청했을 때만 — 직전 「밥은 먹었어?」+「위로해줘」에 「동네 몰라서 못 붙였어」가 나갔다.
            거절(그만·됐어)·위치 권한 안내 턴·감정 턴 제외. 「안 찾아볼게」는 약속이 아니다. */
      {
        const um = String(userMsg || "");
        const ll: any = rel?.session_meta?.last_list;
        const foodCtx = ll?.at && (Date.now() - Date.parse(ll.at)) < 15 * 60000 && (ll.items || []).some((x: any) => x?.ctype === "food");
        ...
        const foodNow = /(맛집|먹을\s*(데|곳)|식당|밥집|카페|메뉴|브런치)/.test(um) || MENUS.some((w) => um.includes(w)) || !!foodCtx;
        const homeAsk = /(우리\s*집|집\s*(에서|근처|앞|쪽)|우리\s*동네|사는\s*데)/.test(um);
        const stop = /(그만|말고|하지\s*마|싫어|됐어|필요\s*없)/.test(um);
        const reqish = /(추천|알려|찾아|어디|가까운|근처|맛집|보여|괜찮은\s*데|갈\s*데|먹을\s*데|(으로|로)\s*(줘|해\s*줘)|왜\s*안\s*(줘|주)|언제\s*(줘|줄))/.test(um) && !/(뭐라고\s*했|했지\?|했었|기억)/.test(um);
        const PROMISE = /(?<!안\s{0,2})(찾아\s*볼게|찾아서\s*줄게|골라서\s*줄게|골라\s*줄게|띄워\s*줄게|찾아\s*줄게|찾는\s*중|찾고\s*있어|잠깐만\s*(기다|찾)|몇\s*분\s*(안에|뒤에|만)|곧\s*줄게|금방\s*줄게|바로\s*줄게|알아\s*볼게|(알아보고|알아봐서|정리해서|찾아서)\s*[^.!?\n]{0,8}(줄게|붙여\s*줄게|띄워\s*줄게))/;
        const promise = PROMISE.test(String(reply || ""));
        const QUOTE = /'([^'\n]{2,20})'|「([^」\n]{2,20})」/;
        const talkRec = /(유명하대|맛있대|괜찮대|잘한대|나온대|평이\s*좋|가\s*봐|가\s*볼래|열어\s*봐)/.test(String(reply || "")) || QUOTE.test(String(reply || ""));
        const cookHome = /(해\s*먹|만들어\s*먹|요리|레시피|집밥)/.test(um);
        const eligible = !crisis && !stop && !SOFT && !_nearNoGeo && !cookHome && foodNow && reqish;
        if (eligible && ((!hasCard() && (promise || talkRec)) || homeAsk)) {
          const recentU = (history || []).filter((h: any) => h?.role === "user").slice(-6).map((h: any) => String(h.content || "")).concat(um);
          const ctxU = recentU.join(" ");
          const menu = [...new Set(MENUS.filter((w) => ctxU.includes(w)))].slice(-1);
          /* 동네 낱말 — 동·구·역·천으로 끝나거나 잘 알려진 동네만(「출근길」「카레로」「어디로」가 동네로 잡혔다) */
          const KNOWN = /(강남|홍대|성수|이태원|잠실|여의도|종로|명동|신촌|건대|합정|연남|망원|판교|분당|일산|해운대|서면|광안리|을지로|익선|성북|용산|양재)/;
          let place: string[] = [...new Set<string>([...(ctxU.match(/[가-힣]{2,6}(동|구|역|천)(?=[\s에쪽근처,.!?]|$)/g) || []), ...(ctxU.match(KNOWN) || [])])]
            .filter((w: string) => !/(우리|거기|여기|이쪽|저쪽|근처|가까운|남자친구|여자친구|친구|운동|행동|감동|이동|자동|정구|연구|요구|친구)/.test(w)).slice(-1);
          if (homeAsk || (!place.length && /(우리\s*집|집\s*(에서|근처|앞|쪽)|우리\s*동네|사는\s*데)/.test(ctxU))) {
            try {
          ...
          const plan: [boolean, boolean, string][] = base
            ? [[true, true, ""], [false, true, menu[0] ? `${place[0]} 쪽엔 ${menu[0]} 집이 없어서 근처 다른 데로 — ` : ""]]
            : [[true, false, ""]];
          let found = false;
          for (const [wm, wp, lead] of plan) {
            ...
            /* 이름은 통째로만 — 90자에서 가게 이름 중간이 잘렸다 */
            let names = ""; for (const x of rows) { const n2 = names ? names + ", " + x.name : x.name; if (n2.length > 40 && names) break; names = n2; }
            reply = `${lead}${homeAsk && place[0] && !lead ? `${place[0]} 쪽에서 찾았어 — ` : ""}여기 붙여놨어: ${names}`;
            found = true; _fixed = true; break;
          }
          if (!found && !hasCard()) { _fixed = true; reply = base ? `미안, 갈라 맛집 지도엔 ${place[0]} 쪽 가게가 아직 없어 ㅠ` : `미안, 동네를 몰라서 못 붙였어 ㅠ 어느 동네인지 알려줘`; }
      ...
      {   /* 🏷 맛집 카드 턴에 카드에 없는 가게 이름('현선이네')을 말로 추천 — 그 문장만 뺀다. 「'인생 맛집'」 같은 표현은 가게 이름이 아니다 */
        ...
        const fc = fcA.map((a: any) => String(a.title || "").replace(/\s+/g, ""));
        if (fc.length) {
          ...
          const alien = (sg: string) => [...sg.matchAll(/'([^'\n]{2,20})'|「([^」\n]{2,20})」/g)].some((m) => {
            const nm = String(m[1] || m[2]).replace(/\s+/g, "");
            if (/(맛집|인생|최애|감성|느낌|진리|국룰|꿀팁|분위기|가성비)/.test(nm)) return false;
            return !fc.some((t) => t.includes(nm) || nm.includes(t)) && !said.includes(nm);
          });
    ...
    if (!reply) {
      ...
      reply = _dk ? "초안 나갔어! 편집기에서 같이 다듬자 ㅋㅋ"
        : _cf ? (_cf.op === "bet" ? `${_cf.sub || "준비"} — 확인 누르면 바로 참여돼! 🔥` : `${_cf.title || ""} ${_cf.sub || ""} — 확인 누르면 바로 돼 ㅎㅎ`)
        : _lk ? (String(_lk.title || _lk.sub || "").trim() ? `${/(딴\s*거|다른|또)/.test(String(userMsg || "")) ? "이번엔 이거 — " : "여기 붙여놨어 — "}${String(_lk.title || _lk.sub).slice(0, 40)} ㅋㅋ` : (/(딴\s*거|다른|또)/.test(String(userMsg || "")) ? "다른 걸로 붙여놨어 ㅋㅋ" : "여기 붙여놨어 ㅋㅋ"))
    ...
    //    상대가 콘텐츠를 열어달라 했는데 point_to(view/share) 액션이 없으면(내용만 떠들고 안 엶) → 강제로 열게 재시도.
    {
      ...
      const wantShow = !_v2Talk && !_noPush && !illegal && !jailbreak && userMsg && !work && !rawSources.length &&
        ((_v2State === "request" && !isCreateAsk(userMsg)) ||   // 🎯 요청 턴인데 카드가 없으면 다시 찾게(「여행지 추천해줘」에 말로만 대만 얘기 — 26.9.22)
    ...
    //    (페르소나 금지: 다음 턴에 못 돌아옴) 그 자리서 web_search를 강제 호출→진짜 추천으로 답을 다시 만든다.
    {
      ...
      if (userMsg && !body?.meta && !work && !creatingNow && promisesSearch && !searchHits.length && !actions.some((a) => a.kind === "open")) {
        ...
        try {
          messages.push({ role: "system", content: "너는 방금 '찾아줄게/기다려봐/검색해볼게'라고 '약속만' 했다 — 넌 다음 턴에 스스로 못 돌아온다(= 상대는 영원히 못 받는다, 페르소나 명백 위반). 지금 이 턴에 즉시 web_search를 호출해 실제로 찾아라(맛집·가게는 web_search 말고 galla_browse(section:food)로, 장소=kind:local, 최신사건=news, 후기=blog). 상대가 말한 지역·메뉴로 쿼리를 만들고, 없으면 지역·키워드를 바꿔 한 번 더. 잡담·약속·질문 금지, web_search만." });
    ...
    {
      const _listAsk = /(뭐\s*(있|있어|있냐|없어)|뭐\s*[가-힣]{1,4}\s*있|열려\s*있|추천|골라|목록|리스트|열린|뜨는|뜨거운|인기|요즘|어디\s*(가|갈))/.test(String(userMsg || "")) && !/(하나|한\s*개|한\s*곳|딱)/.test(String(userMsg || ""));
    ...
    let _askPick = false;   // 🔢 추천 여러 개 → 관문 통과 후 안내를 붙인다
    let _offerOne = false;  // 🃏 카드 한 장 → 「ㅇㅇ 하면 띄워줄게」
    ...
    }
    {
      ...
      if (_autoOpen && !actions.some((a: any) => a.auto)) {
        ...
        // 🃏 26.9.22 사장님: 「바로 띄우던 것 → 멋진 카드 보고 사용자가 판단」. 사용자가 '열어/틀어'로 콕 집었고 하나일 때만 연다.
        if (links.length === 1 && _autoStrong) links[0].auto = true;
      }
    ...
    try {
      for (let i = actions.length - 1; i >= 0; i--) {
        ...
        if (nm) {
          ...
          const { data: fp } = await supa.from("food_places").select("id,name,category,rating,address,cover_url,hours").eq("status", "live")
    ...
    if (!crisis && (_v2State === "request" || wantsContent(String(userMsg || ""))) && !actions.some((a: any) => a.kind === "crisis") && !actions.some((a: any) => /^(open|view|weather|news|local)$/.test(String(a.kind || ""))) && /(붙여\s*놨|띄워\s*놨|(?<!나\s{0,2})여기\s*있어|이거\s*봐)/.test(String(reply || ""))) {   // 위기 턴의 「나 여기 있어」를 카드 거짓말로 오인했다(26.9.22 안전 시험)
      reply = /(맛|먹|식당|가게|밥|카페)/.test(String(userMsg || "")) ? "갈라 맛집 지도엔 딱 맞는 데가 아직 없네 ㅠ 어느 동네야?" : "앗 방금 건 제대로 못 붙였어 ㅠ";
    }
    try { await enrichCards(actions as any[]); } catch { /* 꾸미기 실패는 카드 자체를 막지 않는다 */ }
    /* ✨ 갈비스 픽 — 여러 장 중 본문이 콕 집어 말한 카드 하나(골드 테두리·리본). 뚜렷하지 않으면 안 단다. */
    {
      const ls: any[] = actions.filter((x: any) => (x.kind === "view" || x.kind === "open") && x.title);
      if (ls.length >= 2) {
    ...
    reply = String(reply || "").split(/(?<=(?<!\d)[.!?…]|\n)(?=\s|$)/).map((sen) => {
      /* 예측 카드가 붙은 턴이거나 예측 낱말이 있을 때만 — 「역 쪽에서 걸어 가면 5분」이 「참여해 가면」이 됐다(26.9.22 점검) */
      const _predTurn = actions.some((a: any) => a?.kind === "confirm" || a?.ctype === "predict" || /draftPredict/.test(String(a?.kind || "")));
      if (!(_predTurn || /(GP|지피|예측|선택지|적중|배당)/.test(sen)) || /(말\s*걸|전화|통화|육성톡|면상톡|걸어\s*(가|와|갔|왔|다니)|걸어서)/.test(sen)) return sen;
    ...
    if (!actions.some((a: any) => a.auto) && actions.some((a: any) => a.kind === "view" || a.kind === "open")) {
      /* 「문 열었어」「가게 열었어」는 영업 얘기다 — 「문 여기 붙여놨어」로 망가지던 것(26.9.22 점검) */
      reply = String(reply || "").replace(/(?<!(문|가게|매장|식당|카페)\s{0,2})(바로\s*)?(띄웠어|열었어|틀었어|틀어놨어|띄워놨어|열어놨어|열어줬어|틀어줬어|띄워줬어|열어뒀어)/g, "여기 붙여놨어");   // 카드는 붙인 것 — 「띄울 수 있어」는 못 준 것처럼 들렸다(26.9.22 자동 QA)
    }
    ...
    /* 🎬 빈 답 비상문(「잠깐 헷갈렸어」)이 나간 뒤에 다시 찾기로 카드가 붙은 경우 — 카드를 소개하는 말로 바꾼다
       (26.9.22 검증: 「다른 거」에 카드는 왔는데 「뭐라 해야 할지 잠깐 헷갈렸어」) */
    {
      const _lk2: any = actions.find((a: any) => (a.kind === "open" || a.kind === "view") && String(a.title || "").trim());
      if (_lk2 && /잠깐\s*헷갈렸어/.test(String(reply || ""))) {
        reply = (/(딴\s*거|다른|또|더)/.test(String(userMsg || "")) ? "이번엔 이거 — " : "여기 붙여놨어 — ") + String(_lk2.title).slice(0, 40) + " ㅋㅋ";
      }
    }
    ...
    {   /* 🧹 관문 뒤 최종 정리 — 관문이 카드를 붙일 수 있으니 중복 제거·장수 제한은 여기서 한 번 더(점검: 같은 가게 두 장) */
      ...
      for (let k = 0; k < actions.length; k++) {
        ...
        const key = `${a.kind}:${a.ctype || ""}:${a.id || a.url || a.title}`;
        if (seen.has(key)) { actions.splice(k, 1); k--; } else seen.add(key);
      }
      let n = 0; for (let k = 0; k < actions.length; k++) { const a: any = actions[k]; if (a?.kind === "view" || a?.kind === "open") { n++; if (n > 5) { actions.splice(k, 1); k--; } } }
    }
    /* 🔢 보여준 목록 기억(15분) — ⚠️ 관문·최종 정리 **뒤**에서 저장한다(앞에서 저장하면 화면에 없던 카드가 「2번」으로 열렸다, 26.9.22 점검) — 다음 턴 「2번 거기/제일 뜨거운 거/그 두 번째」를 서버가 알아듣게(26.9.21 QA: "아직 아무것도 못 보여줬는데") */
    {
