/* ═══════════════════════════════════════════════════════════════
 * GALLA SPA 뷰 어댑터 — travel-place (여행지 상세, 스택 뷰)
 *
 * 로직은 js/travel-place.js 가 전부 갖고 있고 여기서는 그 스크립트를 1회 주입한 뒤
 * window.GALLA_PAGE_TRAVEL_PLACE.mount(root, params) 에 위임만 한다(광장 상세와 같은 계약).
 * ⚠️ 뷰 로더는 HTML 안의 <script> 를 버린다 — 여기 배열에 없으면 앱에서 화면이 통째로 빈다.
 * ═══════════════════════════════════════════════════════════════ */
const V = window.GALLA_V ? "?v=" + window.GALLA_V : "";
const SCRIPTS = [
  "/js/travel-place.js",   // 본체
...
export async function mount(root, params) {
  ...
  const api = window.GALLA_PAGE_TRAVEL_PLACE;
