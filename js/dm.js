...
(function () {
  ...
  function buildRoot() {
        ...
        <div class="dm-view" data-view="inbox">
          <div class="dm-head has-btns dm-head-brand">
            ...
            <span class="dm-head-btns">
              ...
              ${PAGE_MODE() ? '' : `<button class="dm-x-close" data-act="close" aria-label="닫기">${ICONS.x}</button>`}
            </span>
          </div>
    ...
    if (!window.__dmDownHook) { window.__dmDownHook = true; document.addEventListener('pointerdown', (ev) => { window.__dmDownT = Date.now(); window.__dmDownEl = ev.target; }, { capture: true, passive: true }); }
    ...
    ROOT.addEventListener('click', async e => {
      ...
      if (cb) {
        ...
        // 👻 유령 근본 차단: '이 버튼에서 시작된 pointerdown'이 있어야만 발신한다.
        //   터치 관통(통화 오버레이의 끊기 버튼에서 눌렀다가 오버레이가 사라져 밑의 '다시 걸기'로 click이 떨어짐)이나
  ...
  /* ---------- 👤 친구 프로필 (카톡 프로필 화면 문법) ----------
     ...
     갈라 성향·전적)를 보여준다. 채팅은 여기의 '메시지' 버튼으로. */
  let PROF_TOKEN = 0;
