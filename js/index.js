...
function openShorts(list, issueId) {
  ...
  SHORTS_STATE.list = list;
  SHORTS_STATE.index = list.findIndex(i => i.id === issueId);

  if (SHORTS_STATE.index < 0) {
    console.error("[SHORTS] issue not found in list");
    return;
  }
  ...
  // 🔥 shorts.js 진입 포인트
  if (typeof window.initShorts === "function") {
    window.initShorts(SHORTS_STATE);
  }
}
