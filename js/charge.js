...
window.GALLA_PORTONE = {
  ...
  /* ⚠️ 2026-09-08 비움 — 웹 PG 결제를 닫는다. 사유:
     ① KG이니시스가 갈라를 '포인트충전(환금성 업종)'으로 분류 → 간편결제 불가·카드사 한도
        (신한 일 5만원 등)·보증보험 월정산 6배·심사 2개월. 1차 출시는 IAP 로만 간다(사장님 확정).
     ② 그리고 여기 테스트 채널키가 박혀 있는 동안 galla.im 에서 **아무나 충전이 됐다**.
        이니시스 테스트 MID 는 실승인이 난다(실측: 1,000원 승인 문자 수신). 정산 계약은 없는데
        유저 카드에서는 돈이 나가고 웹훅이 GC 를 지급한다 — 열어두면 안 되는 상태였다.
     실계약이 끝나면 실연동 채널키를 여기 넣으면 그대로 열린다. */
  channelKey: "",
};
