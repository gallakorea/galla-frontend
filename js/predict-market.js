...
function render(){
  ...
  <div class="pb-wrap">
    ...
    <div class="pb-actions">
      ...
      ${!m.created_by ? '' : `<button class="pb-act" data-support data-sp-kind="market" data-sp-id="${esc(String(m.id))}" data-sp-uid="${esc(m.created_by)}" data-sp-name="" aria-label="후원">${window.GALLA_SUPPORT_ICON || ''} 후원</button>`}
...
function renderHero(){
  ...
  const crowd = window.GALLA_PredictCrowd
    ? `<div class="pm-crowd">${window.GALLA_PredictCrowd.html({
        ...
        mine: myOc, mid: STATE?.id || marketId || null, max: isMulti() ? 5 : 2, empty: !(Number(STATE?.participants) > 0),
...
async function placeBet(){
  ...
  try{
    ...
    if(!data.ok){
      ...
      // 🛟 파산 방지 상한 — 왜 막혔는지 숫자로 알려준다. 안 그러면 버그로 오해한다.
      if(data.reason==='stake_cap'){
        toast(`한 번에 보유 GP의 30%까지만 걸 수 있어요 (최대 ${Math.floor(data.cap||0).toLocaleString()} GP)`);
        return;
      }
...
function renderComments(body){
  ...
  $('cmtSend').addEventListener('click',()=>{
    if(needLogin())return;   // 비로그인은 입장 선택 안내보다 로그인이 먼저(26.9.18)
    if(CMT_SIDE==null){
