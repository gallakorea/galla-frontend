...
(function () {
  ...
  /* 대사 창고 — 희로애락이 섞이게(26.9.19 사장님: 「열받아서·웃겨서·슬퍼서 참여하게, 다양하게, 욕도 좋다」).
     욕은 가린 형태(ㅅㅂ·씨X·개빡)로 — 날것 그대로면 스토어 연령등급·심사에 걸린다. */
  const LINES_CALL = {
    pro: ["이쪽이야!", "여기가 맞아!", "너 우리 편이지?", "손 잡아!", "빨리 와!!", "고민할 시간에 당겨!", "우리 편 오면 치킨 쏜다(거짓말)", "제발… 한 명만…",
          "여기 줄 서면 복 받음", "아 빨리 좀 와 씨X", "안 오면 서운해할 거야", "니가 오면 이겨", "눈 마주쳤지? 이리 와", "편 안 고르면 반칙이다", "우리 편 사람 좋아"],
    con: ["우리 편 와!", "이쪽이 진리!", "여기 자리 있어!", "망설이지 마!", "어서 와~", "저쪽 가면 후회한다", "여기가 찐이야", "혼자 구경만 할 거야?",
          "엄마가 이쪽 가랬어", "빨리 안 와? ㅅㅂ 팔 떨어진다", "우리 편은 간식 있음", "와서 한 번만 잡아줘", "너 같은 사람 기다렸어", "구경꾼은 벌금", "딱 너만 오면 돼"],
  };
  const LINES_LOSE = ["아 ㅅㅂ 밀린다", "이게 지네?? 개빡치네", "우리 편 다 어디 갔냐", "눈물 난다 진짜…", "한 명만… 제발 한 명만…", "배신자들 다 나와",
    "아 씨X 손바닥 다 까졌어", "이럴 거면 왜 불렀어ㅠ", "엄마 나 끌려가…", "저놈들 뭐 먹고 왔냐", "억울해서 잠 못 잔다", "아직 안 끝났다 이것들아",
    "살려주세요ㅠㅠ", "내 인생도 이렇게 밀려…", "지원군 언제 와!!", "진짜 개빡친다", "놓으면 끝이야 버텨!!", "나 여기서 죽는다…", "누가 기름 발랐냐 줄에",
    "아 쟤네 반칙하는 거 봤어?", "울면서 당기는 중"];
  const LINES_WIN = ["개꿀ㅋㅋㅋ", "밥은 먹고 왔냐?ㅋ", "이 맛에 줄다리기 하지", "벌써 끝났냐?ㅋㅋ", "힘 좀 써봐 그쪽~", "우리 편 개잘함", "저쪽 표정 봐ㅋㅋ",
  ...
  const LINES_TIE = ["살려주세요ㅠㅠ", "죽을 것 같아…", "팔 빠질 것 같아", "손 놓으면 진다!", "허리 나갔어…", "물 한 잔만…", "누가 내 발 밟았어?!",
    "한 명만 더 오면 이겨!", "영차! 영차!", "끝까지 간다!!", "숨 좀 쉬자…", "엄마 나 여기 있어!", "밀리면 끝이야!", "이거 놓으면 나 진짜 운다",
    "누가 먼저 놓나 보자", "아 ㅅㅂ 팽팽하네", "팔에 쥐 났어ㅠ", "내일 출근 못 함", "방귀 뀌면 진다 참아", "심장 터진다 진짜", "이거 몇 시간째냐",
