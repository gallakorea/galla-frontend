// shorts.js — STABLE + OPTIMIZED (NO FEATURE LOSS)

/* =========================================================
   GLOBAL GUARD & API (ALWAYS AVAILABLE)
========================================================= */
...
/* =========================================================
   SHORTS MODULE (IIFE)
========================================================= */
(function SHORTS_MODULE() {
  if (!IS_SHORTS_PAGE) {
    console.info("[SHORTS] non-shorts page, module skipped");
    return; // ✅ 합법 (IIFE 내부)
  }
  ...
  async function slide(dir) {
    ...
    setTimeout(async () => {
      ...
      if (typeof window.GALLA_CHECK_VOTE === "function") {
        const r = await window.GALLA_CHECK_VOTE(cur.id);
        applyShortsVoteState(r);
      }
    }, 350);
  }
